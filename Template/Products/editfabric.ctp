...
/*PPSASCRUM-83: start*/
echo $this->Form->input('new_image',['label'=>'New Fabric Image','required'=>false,'type'=>'file','accept'=>'image/jpeg, image/png, image/gif']);
/*PPSASCRUM-83: end*/
...
/* PPSASCRUM-358: start */
$pfasStatusOptions = ['free-of-pfas' => 'Free of PFAS', 'contains-pfas' => 'Contains PFAS', 'not-verified' => 'Not Verified'];
$pfasStatusMap = array_flip($pfasStatusOptions);
echo $this->Form->input('pfas_status', ['type' => 'select', 'options' => $pfasStatusOptions, 'label' => 'PFAS Status', 'value' => $pfasStatusMap[$fabricData['pfas_status']]]);
/* PPSASCRUM-358: end */
...
$('input[type=file][name=new_image]').change(function (event) {
	...
	/* PPSASCRUM-358: start */
	var fabricPfasStatus = "<?php echo $pfasStatusMap[$fabricData['pfas_status']]; ?>";
	if (!$('select#pfas-status option[selected=selected]').val()) {
		$('select#pfas-status option').each(function(index, element) {
			let currentPfasStatus = $(element);
			if (currentPfasStatus.val() == fabricPfasStatus) {
...
<style>
	...
	/*PPSASCRUM-83: start*/
	div.input div.error{ display:inline-block; margin: 10px 0 20px 10px; background:#F9F3B7; border:2px solid red; color:red; padding:10px; font-size:12px; font-weight:bold; }
	div button[type=submit]:disabled,
	div button[type=submit][disabled]{ cursor:not-allowed !important; background:#444 !important; color:#CCC !important; }
	/*PPSASCRUM-83: end*/
</style>