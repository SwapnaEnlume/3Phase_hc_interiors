...
echo "</th>
<th width=\"8%\">WO LINE</th>
<th width=\"8%\">SO LINE</th>
<th width=\"10%\">QTY<br>ORDERED</th>
<th width=\"10%\">PREV<br>SCHEDULED</th>
<th width=\"14%\">QTY TO<br>SCHEDULE</th>
<th width=\"34%\" colspan=\"4\">DESCRIPTION</th>
</tr>
</thead>";
...
foreach($lineItems as $itemID => $itemData){
	...
	<td width=\"8%\">".$itemData['data']['wo_line_number']."</td>
	<td width=\"8%\">".$itemData['data']['line_number']."</td>
	<td width=\"10%\">".floatval($itemData['data']['qty'])."</td>
	<td width=\"10%\">".floatval($itemData['previously_scheduled'])."</td>
	<td width=\"14%\">";
...
table#schedulebuild tbody tr.allcompleted td:nth-of-type(5){ background:#34C533 !important; }
...
table#schedulebuild tbody tr.duplicated td:nth-of-type(5){ background:#FF0000 !important; }

table#schedulebuild tbody > tr.notscheduling > td{ background:rgba(0,0,0,0.3); }
table#schedulebuild tbody tr td:nth-of-type(11){ background:#FFFFFF !important; }
...
function toggleThisLineVisibility(itemID){
	$.get('/quotes/changelinetally/'+itemID+'/0',
		function(response){
			if(response == 'OK'){
	...
	$('form').submit(function(e){
		...
		$('#schedulebuild > tbody > tr').each(function(){
			if($(this).find('input[type=checkbox]').is(':checked')){
				var thislinenumber=$(this).find('td:nth-of-type(3)').text();
				totalCheckedRows++;
