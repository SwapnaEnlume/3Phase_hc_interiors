...
table#sherrytable{ width:100% !important;  max-width:100%;}
...
<div id="sherryschedule">
	...
	$(function(){
		...
		/**PPSASCRUM-08 start **/
		 ...
		 if( document.referrer.indexOf('/orders/vieworderschedule/#sherryschedule/viewBatch')){
           var currentUrl = window.location.href;
          // alert(currentUrl.indexOf('sherryschedule/viewBatch') );
           if(currentUrl.indexOf('sherryschedule/viewBatch') > -1){
               var params = currentUrl.split('/');
               var len =params.length;
               var fdate = params[7];
               const date = new Date(fdate * 1000);
                batchId =  params[8];
                 
                if(typeof batchId === "undefined" || batchId != "")
    			 changeCalendarDateRange(date.toISOString().substring(0, 10),date.toISOString().substring(0, 10),true,batchId);
		...
		if(currentUrl.indexOf('sherryschedule/viewBatch') > -1){
		      var params = currentUrl.split('/');
               ...
               const date = new Date(fdate * 1000);
                batchId =  params[8];
		    	$.get('/orders/getsherryschedulerows/'+date.toISOString().substring(0, 10)+'/'+date.toISOString().substring(0, 10)+'/'+$('#highlightrowids').val(),function(responseText){
			$('#scriptsoutput').html(responseText);
			$.fancybox.hideLoading();
