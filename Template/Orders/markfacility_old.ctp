...
echo "<a href=\"#\" id=\"new_address\"  style=\"display: block; height: 36px; color: white; width: 20%; text-align: center; padding: 6px 0px 0px; background: rgb(71, 1, 1) !important; float: right !important;\" id='new_address'>+ New Address </a>";
...
$("#new_address").on('click', function() {
     var prevVal =  ($('#facility_name').val().length != 0) ? $('#attention').val() +' :|: ' +$('#facility_code').val()+" :|: "+escapeTextFieldforURL($('#facility_name').val()) : '';
     //var prevVal =  escapeTextFieldforURL($('#attention').val() +':' +$('#facility_code').val()+":"+$('#facility_name').val());
...
function reverseTextFeildforURL(thetext){
    if(thetext !== "undefined" && thetext.length > 0){
