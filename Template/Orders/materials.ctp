							...
							if($fabricid==$fabric['fabric_id']){
								foreach($data['yardages'] as $num => $linedata){
									if(!in_array($linedata['order_number'],$usedWOsThisMOMFabric)){
										...
										echo "</td><td valign=\"top\"><a href=\"/orders/editlines/".$linedata['order_id']."/workorder?highlightFabric=".$fabric['id']."\" target=\"_blank\" title=\"View Order Details with Highlighting\">".$linedata['order_number']."</a></td><td valign=\"top\">".$linedata['po_number']."</td></tr>";
		...
		<table width="100%" id="purchasetable" cellpadding="5" cellspacing="0" border="1" bordercolor="#000000">
			...
			<tbody>
				...
				foreach($fabricOrders as $fabricOrder){
					...
					foreach($wo as $worow){
						echo "<li>";
						echo "<a href=\"/orders/editlines/workorder".$worow."?scrolltobom=1\" target=\"_blank\">".$wonums[$worow]."</a>";
						echo "</li>\n";
					}
					echo "</ul>";
					echo "</td>
