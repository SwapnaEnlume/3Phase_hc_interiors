...
/**PPSASCRUM-3 start **/
//print_r($thisCustomer['on_credit_hold']);
$creditHold =  ($thisCustomer['on_credit_hold']) ? '<span style="color: red;"> On Credit Hold</span>  ' : ' ';

echo "<h1>" .$creditHold."  Manage <u>Order ".$thisOrder['order_number']."</u> Scheduling</h1>";
/**PPSASCRUM-3 end **/
...
table#scheduledtable > tbody > tr > td:nth-of-type(8){ padding:0 0 0 0 !important; }
table#scheduledtable > tbody > tr > td:nth-of-type(8) > table > thead > tr{ background:rgba(0,0,0,0.15) !important; color:#111 !important; }
...
table#scheduledtable > tbody > tr > td:nth-of-type(8) > table > thead > tr > th{ color:#111 !important; padding:2px 2px 2px 2px !important; font-weight:normal !important; text-transform:uppercase; font-size:11px; line-height:1.0 !important; }
table#scheduledtable > tbody > tr > td:nth-of-type(8) > table > tbody > tr.batchrow > td{ padding:1px 1px 1px 1px !important; font-size:13px; border-top:5px solid #26337A; }
table#scheduledtable > tbody > tr > td:nth-of-type(8) > table > tbody > tr.batchrow:nth-of-type(1) > td{ border-top:1px solid #444 !important; }

table#scheduledtable > tbody > tr > td:nth-of-type(8) > table > tbody > tr > td:nth-of-type(6){ text-align:left !important; font-size:11px; }
...
table#scheduledtable > tbody > tr > td:nth-of-type(8) > table > tbody > tr.batchrow > td{ text-align:center !important; }

table#scheduledtable > tbody > tr > td:nth-of-type(8) > table{ background:transparent !important; margin-bottom:0 !important; }
...
table#scheduledtablewithboxes > tbody > tr > td:nth-of-type(8){ padding:0 0 0 0 !important; }
table#scheduledtablewithboxes > tbody > tr > td:nth-of-type(8) > table > thead > tr{ background:rgba(0,0,0,0.15) !important; color:#111 !important; }
...
table#scheduledtablewithboxes > tbody > tr > td:nth-of-type(8) > table > thead > tr > th{ color:#111 !important; padding:2px 2px 2px 2px !important; font-weight:normal !important; text-transform:uppercase; font-size:11px; line-height:1.0 !important; }
table#scheduledtablewithboxes > tbody > tr > td:nth-of-type(8) > table > tbody > tr.batchrow > td{ padding:1px 1px 1px 1px !important; font-size:13px; border-top:5px solid #26337A; }
table#scheduledtablewithboxes > tbody > tr > td:nth-of-type(8) > table > tbody > tr.batchrow:nth-of-type(1) > td{ border-top:1px solid #444 !important; }

table#scheduledtablewithboxes > tbody > tr > td:nth-of-type(8) > table > tbody > tr > td:nth-of-type(6){ text-align:left !important; font-size:11px; }
...
table#scheduledtablewithboxes > tbody > tr > td:nth-of-type(8) > table > tbody > tr.batchrow > td{ text-align:center !important; }

table#scheduledtablewithboxes > tbody > tr > td:nth-of-type(8) > table{ background:transparent !important; margin-bottom:0 !important; }
...
if(count($allBatchesThisWO) == 0){
	...
	echo "<table width=\"100%\" cellpadding=\"5\" cellspacing=\"0\" border=\"1\" id=\"scheduledtable\">
	<thead>
		<tr>
		    <th width=\"6%\" align=\"center\">WO Line#</th>
			<th width=\"3%\" align=\"center\">SO Line#</th>
...
foreach($allUnscheduledItems as $item){
	if($item['outstanding_qty'] > 0){
		...
		foreach($orderLines as $orderLine){
		  //  print_r($orderLine); print_r($quoteLine);
		  // echo $orderLine['quote_line_item_id'] . "--->".$quoteLine['id']. '---->'.$quoteLine['quote_line_item_id']."\n";
			if(($orderLine['quote_line_item_id'] == $quoteLine['id'])){
				$thisOrderItemID=$orderLine['id']; 
			}
		}
		echo "<tr class=\"lineitemrow\">
		<td width=\"6%\" align=\"center\" valign=\"top\">".$quoteLine['wo_line_number']."</td>
		<td width=\"3%\" align=\"center\" valign=\"top\">".$quoteLine['line_number']."</td>
		<td width=\"5%\" valign=\"top\">".$quoteLine['room_number']."</td>
	...
	echo ($allUnscheduledItems[$thisOrderItemID]['scheduled'] + $allUnscheduledItems[$thisOrderItemID]['outstanding_qty']);
		...
		echo "</b><br>";
		/* PPSASCRUM-160: start */
		if(isset($quoteLine['metadata']['com-fabric']) && $quoteLine['metadata']['com-fabric'] == 1) {
			echo 'COM &nbsp;';
		}
		/* PPSASCRUM-160: end */
		echo $quoteLine['fabricdata']['fabric_name'].'&nbsp;&nbsp;&nbsp;'.$quoteLine['fabricdata']['color'];
    ...
    //     echo "</pre>";
 			...
 			if($thisScheduledOrderLineItemID == $thisOrderItemID){
				...
				foreach($scheduledItemRow as $batchID => $batchTotals){
					...
					echo "</td>
					<td width=\"13%\" style=\"border-right: hidden !important;\"";
					...
					if(isset($allCompletedItems[$thisScheduledOrderLineItemID][$batchID]) && intval($allCompletedItems[$thisScheduledOrderLineItemID][$batchID]['qty']) == intval($batchTotals['qty'])){
						...
						if(isset($allShippedItems[$thisScheduledOrderLineItemID][$batchID]) && intval($allShippedItems[$thisScheduledOrderLineItemID][$batchID]['qty']) == intval($batchTotals['qty'])){
							...
							echo "</td><td width=\"12%\" class=\"shippeditem\">";
							...
							echo " class=\"completeditem\">COMPLETED</td><td width=\"12%\" class=\"completeditem\">";
						...
						echo " class=\"scheduleditem\">SCHEDULED</td><td width=\"12%\" class=\"scheduleditem\">".$batchTotals['date'];
						$isScheduled++;
					}
					echo "</td>
					<td width=\"35%\">";
					...
					<td width=\"10%\">";
					//PPSASCRUM-08 start
				     echo "<a href=\"/orders/viewbatchschedule/".$batchID."\" target=\"_blank\"><img src=\"/img/view.png\" alt=\"View This Batch\" title=\"View This Batch\" /></a>";
				    //PPSASCRUM-08 end
                    ...
                    //PPSASCRUM-08 start
                    $startDate = strtotime($batchTotals['date'].' 08:00:00');
                  // $startDate =  strtotime($batchTotals['date'].' 08:00:00');
                    echo "<a href='/orders/schedule/#sherryschedule/viewBatch/".$startDate."/".$batchID ."' target=\"_blank\">Go</a> ";
                    //PPSASCRUM-08 end
					echo "</td>
					</tr>";
...
echo "<table width=\"100%\" cellpadding=\"4\" cellspacing=\"0\" border=\"1\" id=\"scheduledtablewithboxes\">
	<thead>
		<tr>
			...
			<th width=\"2%\" align=\"center\">Line#</th>
...
foreach($allUnscheduledItems as $item){
	if($item['outstanding_qty'] > 0){
		...
		echo "<tr class=\"lineitemrow\">
		<td width=\"4%\" align=\"center\" valign=\"top\">".$quoteLine['wo_line_number']."</td>
		<td width=\"2%\" align=\"center\" valign=\"top\">".$quoteLine['line_number']."</td>
		<td width=\"4%\" valign=\"top\">".$quoteLine['room_number']."</td>
		<td width=\"4%\" valign=\"top\" align=\"center\">";
	    
		echo 
	$allUnscheduledItems[$thisOrderItemID]['outstanding_qty'] ;
