...
/* PPSASCRUM-160: start */
$momcheckboxArr=['label'=>'MOM Fabric Special Cost p/yd','type'=>'checkbox'];
if($mode=='edit' && (
   (isset($lineitemmeta['fabric-cost-per-yard-custom-value']) && !empty($lineitemmeta['fabric-cost-per-yard-custom-value'])) ||
   (isset($lineitemmeta['mom-fabric']) && $lineitemmeta['mom-fabric'] == '1')
)) {
    $momcheckboxArr['checked'] = true;
    $momcheckboxArr['value'] = 1;
} else {
    $momcheckboxArr['checked'] = false;
    $momcheckboxArr['value'] = 0;
}
/* PPSASCRUM-160: end */
...
<td width=\"50%\" valign=\"top\" style=\"padding:15px;\">
    ...
    */
    ...
    //&& is_numeric($lineitemmeta['pinset'])
    if(isset($lineitemmeta['pinset']) ){
...
$(function(){
    ...
    /* PPSASCRUM-160: start */
    if ($('#mom-fabric').is(':checked')) {
        $('#mom-fabric').val(1);
        $('#com-fabric').prop('disabled', true);
    } else {
        $('#mom-fabric').val(0);
        $('#com-fabric').prop('disabled', false);
    }
    if ($('#com-fabric').is(':checked')) {
        $('#com-fabric').val(1);
        $('#mom-fabric').prop('disabled', true);
    } else {
        $('#com-fabric').val(0);
        $('#mom-fabric').prop('disabled', false);
    }
    $('#mom-fabric').change(function() {
        if ($(this).is(':checked')) {
            $(this).val(1);
            $('#com-fabric').prop('disabled', true);
        } else {
