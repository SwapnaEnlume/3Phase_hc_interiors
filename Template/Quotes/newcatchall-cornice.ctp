...
if($mode=='edit'){
    ...
    echo "<h3>Edit ".$typeTitle."Cornice Catch-All Line Item</h3>";
}else{
    echo "<h3>Add Cornice Catch-All Line to Quote</h3>";
}
...
echo "<div id=\"fabric-selector-none\"";
...
<td width=\"50%\" valign=\"top\" style=\"padding:15px;\">
    ...
    if(isset($lineitemmeta['cornice-type']) && strlen(trim($lineitemmeta['cornice-type'])) >0){
    	$cornicetypeval=$lineitemmeta['cornice-type'];
    }else{
    	$cornicetypeval='';
    }
    ...
    echo $this->Form->select('cornice-type',[
    	...
    	],['id'=>'cornice-type','value'=>$cornicetypeval,'empty'=>'--Select--']);
    echo "</div>";
    ...
    echo $this->Form->input('hingecount',['label'=>'Hinge Count','type'=>'number','min'=>0,'step'=>1,'value'=>$hingecountval]);
    echo "</div>";
    ...
    if(isset($lineitemmeta['welt-top']) && $lineitemmeta['welt-top']=='1'){
    	$welttopChecked=true;
    }else{
    ...
    if(isset($lineitemmeta['welt-bottom']) && $lineitemmeta['welt-bottom']=='1'){
    ...
    <td width=\"50%\" valign=\"top\" bgcolor=\"#EEEEEE\" style=\"padding:15px;\">
        ...
        if(isset($lineitemmeta['cornice-widths-fabric']) && strlen(trim($lineitemmeta['cornice-widths-fabric'])) > 0){
        	$cornicewidthsfabricval=$lineitemmeta['cornice-widths-fabric'];
        }else{
        	$cornicewidthsfabricval='';
        }	
        echo $this->Form->input('cornice-widths-fabric',['label'=>'CORNICE WIDTHS FABRIC','type'=>'number','step'=>'0.5','value'=>$cornicewidthsfabricval]);
        	


        
        //total widths field *autocalc
        if(isset($lineitemmeta['total-widths']) && strlen(trim($lineitemmeta['total-widths'])) > 0){
        	$totalwidthsval=$lineitemmeta['layout'];
        }else{
        	$totalwidthsval='';
        }
        echo $this->Form->input('total-widths',['label'=>'Total Fabric Widths','readonly'=>true,'value'=>$totalwidthsval]);
        ...
        echo $this->Form->input('yds-per-unit',['label'=>'Yards of Fabric ea','value'=>$ydspbsval, 'required'=>true]);
...
function checkSubmittable(){
    ...
    if($('#qty').val() != '' && $('#cornice-widths-fabric').val() != ''){
        $('#total-widths').val( ( parseFloat($('#cornice-widths-fabric').val()) * parseInt($('#qty').val()) ) );   
    }else{
        $('#total-widths').val('');
    }
