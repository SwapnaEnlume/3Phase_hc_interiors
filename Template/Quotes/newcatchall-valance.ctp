...
<?php
if($mode=='edit'){
                    $typeTitle = ($ordermode == 'workorder') ? "WorkOrder ": ($ordermode == 'workorder') ? "SalesOrder " : " ";

    echo "<h3>Edit ".$typeTitle."  Valance Catch-All Line Item</h3>";
}else{
    echo "<h3>Add Valance Catch-All Line to Quote</h3>";
}
...
<td width=\"50%\" valign=\"top\" style=\"padding:15px;\">
    ...
    if(isset($lineitemmeta['valance-type']) && strlen($lineitemmeta['valance-type']) > 0){
        $valTypeVal=$lineitemmeta['valance-type'];
    }else{
        $valTypeVal='';
    }
    
    echo $this->Form->input('valance-type',['type'=>'text','label'=>'Valance Type','required'=>true, 'value' => $valTypeVal]);
    ...
    echo $this->Form->input('face',['label'=>'Face','type'=>'number','min'=>0,'step'=>'any','value'=>$faceval,'required'=>true]);
    
    
    if(isset($lineitemmeta['hinged']) && $lineitemmeta['hinged']=='1'){
    	$hingedChecked=true;
    }else{
    	$hingedChecked=false;
    }
    ...
    echo "<div id=\"hingecountwrap\" class=\"suboptions\">";
    if(isset($lineitemmeta['hingecount']) && is_numeric($lineitemmeta['hingecount'])){
    	$hingecountval=$lineitemmeta['hingecount'];
    }else{
    	$hingecountval='';
    }
    ...
    echo "<div id=\"flmidwrap\">";
    if(isset($lineitemmeta['fl-mid']) && is_numeric($lineitemmeta['fl-mid'])){
    	$flmidval=$lineitemmeta['fl-mid'];
    }else{
    	$flmidval='';
    }
    ...
    echo $this->Form->input('height',['label'=>'Height (Long Point)','type'=>'number','min'=>0,'step'=>'any','value'=>$heightval, 'required'=>true]);
    ...
    echo $this->Form->input('return',['label'=>'Return','type'=>'number','min'=>0,'step'=>'any','value'=>$returnval, 'required'=>true]);
    
    
        
    if(isset($lineitemmeta['pleats']) && is_numeric($lineitemmeta['pleats'])){
    	$pleatsval=$lineitemmeta['pleats'];
    }else{
    	$pleatsval='';
    }
    ...
    echo "<label>Lining <span style=\"content: ' *';color: #C3232D;font-weight:bold\">*</span></label>";
    echo "<select name=\"linings_id\" id=\"linings_id\" required=\"true\"><option value=\"\" data-price=\"0.00\">--Select Lining--</option><option value=\"none\"";
    if($lineitemmeta['linings_id'] == "none"){
    		echo " selected=\"selected\"";
    	}
    
    echo " data-price=\"0.00\">No Lining</option>";
    ...
    <td width=\"50%\" valign=\"top\" bgcolor=\"#EEEEEE\" style=\"padding:15px;\">
        ...
        if(isset($lineitemmeta['labor-billable']) && strlen(trim($lineitemmeta['labor-billable'])) > 0){
        	$laborbillablelfval=$lineitemmeta['labor-billable'];
        }else{
        	$laborbillablelfval='';
        }	
        echo $this->Form->input('labor-billable',['type'=>'number','step'=>1,'label'=>'Labor Billable LF','value'=>$laborbillablelfval]);
        ...
        echo $this->Form->input('fabric-widths-each',['label'=>'Valance Fabric Widths','value'=>$fabricwidthseachval]);
        ...
        if(isset($lineitemmeta['yds-per-unit']) && strlen(trim($lineitemmeta['yds-per-unit'])) > 0){
        	$ydseaval=$lineitemmeta['yds-per-unit'];
        }else{
        	$ydseaval='';
        }
        echo $this->Form->input('yds-per-unit',['label'=>'Fabric Yards ea','value'=>$ydseaval, 'required'=>true]);
        ...
        /**PPSACRUM-13 start **/
         //linings yds field
        if(isset($lineitemmeta['lining-per-unit']) && strlen(trim($lineitemmeta['lining-per-unit'])) > 0){
        	$liningseaval=$lineitemmeta['lining-per-unit'];
        }else{
        	$liningseaval='';
        }
        echo $this->Form->input('lining-per-unit',['label'=>'Linings Yards ea','value'=>$liningseaval, 'required'=>true]);
        
         //total yds field
        if(isset($lineitemmeta['linings-yds']) && strlen(trim($lineitemmeta['linings-yds'])) > 0){
        	$liningsydsval=$lineitemmeta['linings-yds'];
        }else{
        	$liningsydsval='';
        }
        echo $this->Form->input('linings-yds',['label'=>'Linings Yards Total','readonly'=>true,'value'=>$liningsydsval]);
        /*PPSASCRUM-13 end*/
...
function checkSubmittable(){
    ...
    if($('#valance-type').val() == ''){
        missingCount++;
        $('#valance-type').addClass('stillneeded');
    }else{
        $('#valance-type').removeClass('stillneeded');
    }
    ...
    /**PPSASCRUM-13 start **/
    if($('#qty').val() != '' && $('#lining-per-unit').val() != ''){
        $('#linings-yds').val( ( parseFloat($('#lining-per-unit').val()) * parseInt($('#qty').val()) ).toFixed(2) );
    }else{
        $('#linings-yds').val('');
    }
   /**PPSASCRUM-13 end **/
   ...
   /* if(missingCount == 0){
    ...
    /*PPSA-33 start */
        ...
        /**PPSASCRUM-13 Start **/
         if($('#linings_id').val() == ''){
            missingCount++;
            $('#linings_id').addClass('stillneeded');
        }else{
            $('#linings_id').removeClass('stillneeded');
        }
        
        if($('#lining-per-unit').val() == ''){
        missingCount++;
        $('#lining-per-unit').addClass('stillneeded');
    }else{
        $('#lining-per-unit').removeClass('stillneeded');
    }
        /** PPSASCRUM-13 End **/
     /*PPSASCRUM-201 start
