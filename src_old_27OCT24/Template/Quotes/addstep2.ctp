...
echo "<div id=\"newquoteheading\">
	...
	if($ordermode=='order' || $ordermode=='workorder' || $mode=='editorderlines'){
		...
		if( $mode=='workorder' || $ordermode =='workorder'){
			echo "<h1 class=\"pageheading\">Work Orders</h1>";
			...
			echo "<h1 class=\"pageheading\">Sales Orders</h1>";
...
function deleteLineItem(lineitemid){
	if(confirm('Are you sure you want to delete this line item?')){
		$.fancybox.showLoading();
		$.get('/quotes/deletelineitem/'+lineitemid+'/'+'/'+$('#ordermode').val(),function(data){
			if(data=="OK"){
				updateQuoteTable();
				$.fancybox.hideLoading();
			}
