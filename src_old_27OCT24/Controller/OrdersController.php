...
/**PPSACRUM-7 end **/
	...
	public function editlines($orderid, $ordermode='order'){
		...
		if($thisOrder['status'] == 'Editing'){
		    ...
		    $lookupNewQuote=$this->Quotes->find('all',['conditions' => ['order_id' => $thisOrder['id'], 'status' => 'editorder']])->toArray();
...
/**PPSASCRUM-29 end */
		...
		foreach($lineItems as $lineitem){
			...
			$lineItemMetas=$this->WorkOrderLineItems->find('all',['conditions'=>['quote_id'=>$thisOrder['quote_id']],'order'=>['sortorder'=>'asc']])->toArray();
			/**PPSASCRUM-29 end */
			$lineItemMetaArray=array();
			foreach($lineItemMetas as $lineitemmeta){
				$lineItemMetaArray[$lineitemmeta['meta_key']]=$lineitemmeta['meta_value'];
...
if(!empty($lineitemmeta['meta_value'])){
		...
		if($this->request->data){
		    ...
		    if(count($boxLookup) > 0){
    			...
    			foreach($quoteLineItems as $lineItem){
    				...
    				$itemMetas=$this->WorkOrderLineItemMeta->find('all',['conditions'=>['quote_item_id' => $lineItem['id']]])->toArray();
	...
	public function vieworderschedule($orderID){
		...
		$customerData=$this->Customers->find('all',['conditions'=>['id'=>$thisOrder['customer_id']]])->toArray();
        $this->set('thisCustomer',$customerData);
        /**PPSASCRUM-3 end **/
        
		$orderLines=$this->WorkOrderItems->find('all',['conditions'=>['order_id'=>$orderID]])->toArray();
		$quoteLines=$this->WorkOrderLineItems->find('all',['conditions'=>['order_id'=>$orderID],'order'=>['sortorder'=>'asc']])->toArray();
		...
		foreach($quoteLines as $quoteLineData){
			...
			$lineItemMetas=$this->WorkOrderLineItemMeta->find('all',['conditions'=>['worder_item_id' => $quoteLineData['id']]])->toArray();
...
/**PPSASCRUM-3 start **/
	...
	public function editschedule($batchID){
		...
		if($this->request->data){		
			...
			/**PPSASCRUM-29 start */
			$sherryWorkTable=TableRegistry::get('WorkOrderItemStatus');
					...
					if(count($scheduleCheck) == 1){
						if(intval($val) == 0){
							...
							foreach($scheduleCheck as $scheduleRow){
								...
								$sherryWorkTable->delete($thisScheduleEntry);
								/**PPSASCRUM-29 end */
							}
							...
							foreach($scheduleCheck as $scheduleRow){
								...
								$sherryWorkTable->save($thisScheduleEntry);
								/**PPSASCRUM-29 end */
							}
						}
		...
		if($this->request->data){
            ...
            */
    		...
    		foreach($lineItems as $lineitem){
    			...
    			$lineItemsArray[$lineitem['id']]['metadata']=array();
    			$lineItemsArray[$lineitem['id']]['fabricdata']=array();
    			$lineItemMetas=$this->WorkOrderLineItemMeta->find('all',['conditions'=>['quote_item_id'=>$lineitem['id']]])->toArray();
    			foreach($lineItemMetas as $lineitemmeta){
    				$lineItemsArray[$lineitem['id']]['metadata'][$lineitemmeta['meta_key']]=$lineitemmeta['meta_value'];	
    				if($lineitemmeta['meta_key']=='fabricid' && intval($lineitemmeta['meta_value']) > 0){
    					$thisFabric=$this->Fabrics->get($lineitemmeta['meta_value'])->toArray();
	...
	public function batchboxes($batchID){
	    ...
	    $lineItems=$this->WorkOrderLineItems->find('all',['conditions'=>['order_id'=>$thisWO['order_id']],'order'=>['sortorder'=>'asc']])->toArray();
...
exit;
	...
	public function getShipAddressNameDetails($name){
	    ...
	    $conditions['OR'] += array('ship_to_name LIKE'=>'%'.trim($this->cleanfacilityparameterreplacements($name)).'%');
		$shipto= $this->ShipTo->find('all', ['conditions'=>$conditions])->first();
