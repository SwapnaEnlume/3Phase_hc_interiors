...
application/x-httpd-php QuotesController.php ( C++ source, ASCII text, with very long lines )
<?php
...
class QuotesController extends AppController{
	...
	public function updatevals($quoteID,$mode){
		...
		$lineItemMetaLookup=$this->QuoteLineItemMeta->find('all',['conditions'=>['quote_item_id in'=>$quoteItemsformetavalues, 'meta_key'=>'fabric-cost-per-yard-custom-value','meta_value <>' => '']]);
		...
		foreach($quoteItems as $quoteItem){
			...
			*/
			...
			<td valign=\"top\" width=\"8%\" class=\"actionscol\">";
			...
			$out .= "<td valign=\"top\" width=\"3%\" class=\"linenumber\">";
			...
			$out .= "<td valign=\"top\" style=\"text-align:center\" width=\"3%\">";
			...
			if($quoteItem['product_type'] == 'track_systems'){
              ...
              if( isset($metaArray['com-fabric']) && $metaArray['com-fabric'] == "0" && isset($metaArray['fabric-cost-per-yard-custom-value']) && !empty($metaArray['fabric-cost-per-yard-custom-value'])){
                $out .= "<br/><b style=' color: red;'>Special cost p/yd  $".$metaArray['fabric-cost-per-yard-custom-value']."</b>";
			   }
			/*PPSA-33 end*/
				$out .= "</td>";
			...
			switch($quoteItem['product_type']){
                ...
                case 'newcatchall-valance':
    			    /**PPSACRUM-12 start **/
    			     /*if(isset($metaArray['width'])){
    			        $out .= $metaArray['width']."&quot;";
    			    }*/
    			   
    			    if($quoteItem['product_type'] == 'newcatchall-cubicle' && isset($metaArray['cut-width'])){
						$out .= number_format(floatval($metaArray['cut-width']),0,'','')."&quot; (Cut)";
					} else if(isset($metaArray['width'])){
    			        $out .= $metaArray['width']."&quot;";
    			    }
    			    if($quoteItem['product_type'] == 'newcatchall-cubicle' && isset($metaArray['width']) && strlen(trim($metaArray['width'])) >0){
    						$out .= "<br>".$metaArray['width']."&quot; (Fin)";
    				}elseif ($quoteItem['product_type'] == 'newcatchall-cubicle' && isset($metaArray['width']) && strlen(trim($metaArray['width'])) <0) {
    				    $out .= "<br>0 &quot; (Fin)";
    				}
	    			/**PPSACRUM-12 end **/
			...
			if($metaArray['lineitemtype'] == 'simpleproduct'){
				...
				$out .= $metaArray['fabric-margin']."%";

			}

			$out .= "</td>";

			//end column "fabric margin"

			

			

			//begin column "total fabric profit"
			...
			if($metaArray['lineitemtype'] == 'simpleproduct'){
				...
				$out .= "\$".number_format($metaArray['fabric-profit'],2,'.',',');

			}

			$out .= "</td>";

			//end column "total fabric profit"
			...
			*/
			...
			$out .= "<td valign=\"top\" width=\"4%\">".number_format($bestprice,2,'.',',');
		...
		$out .= "<script>
			...
			/*$('#quoteitems tbody tr td.adjustedprice input').keyup($.debounce( 1000, function(){
...
*/
		...
		<table id=\"quotefooter\" width=\"100%\" cellpadding=\"3\" border=\"0\" cellspacing=\"0\">
			...
			<table style=\"width:2200px !important;\" cellpadding=\"5\" cellspacing=\"0\">
	...
	public function clonequote($quoteID,$isrevision=0,$fromorder=0,$newcustomer=0){
		...
		if($this->request->data){
			...
			if($quotesTable->save($newQuote)){
				...
				$thisQuoteLines=$this->QuoteLineItems->find('all',['conditions'=>['quote_id'=>$quoteID],'order'=>['sortorder'=>'asc']])->toArray();
				foreach($thisQuoteLines as $line){
