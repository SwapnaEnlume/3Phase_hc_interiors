...
class OrdersController extends AppController{
	...
	public function beforeFilter(Event $event) {
		...
		if($this->request->action=='getorderslist' || $this->request->action='getunscheduled' || $this->request->action=='getsherryschedule' || $this->request->action=='packingslipform' || $this->request->action=='addnew' || $this->request->action=='ordermaterial'){

			$this->Security->config('unlockedActions', ['getorderslist','addnew','ordermaterial','getsherryschedule','getunscheduled']);
		...
		foreach($getOrders as $order){
			...
			if($order['status'] == 'Needs Line Items'){
				...
				$orders[]=array(
					...
					'0' => '<a href="/orders/editlines/'.$order['id'].'"/><img src="/img/vieworder.png" alt="Line Items" title="Line Items" /></a> <a href="/orders/edit/'.$order['id'].'"><img src="/img/edit.png" alt="Edit Order Details" title="Edit Order Details" /></a> <a href="/orders/cancelorder/'.$order['id'].'"><img src="/img/cancel.png" alt="Cancel This Order" title="Cancel This Order" /></a> <a href="/quotes/clonequote/'.$order['quote_id'].'/0/1/"><img src="/img/clone.png" alt="Clone This Order to New Draft Quote" title="Clone This Order to New Draft Quote" /></a> <a href="/orders/woboxes/'.$order['id'].'" target="_blank"><img src="/img/box-icon.png" width="24" alt="Work Order Boxes" title="Work Order Boxes" /></a>',
				...
				if($order['status'] == 'Canceled'){
                    ...
                    */
					...
					$actionsout = '<a href="/orders/editlines/'.$order['id'].'"/><img src="/img/vieworder.png" alt="Line Items" title="Line Items" /></a> <a href="/orders/edit/'.$order['id'].'"><img src="/img/edit.png" alt="Edit Order Details" title="Edit Order Details" /></a> <a href="/orders/cancelorder/'.$order['id'].'"><img src="/img/cancel.png" alt="Cancel This Order" title="Cancel This Order" /></a> <a href="/orders/vieworderschedule/'.$order['id'].'" target="_blank"><img src="/img/view.png" alt="Batch Breakdown" title="Batch Breakdown" /></a> <a href="/quotes/clonequote/'.$order['quote_id'].'/0/1/"><img src="/img/clone.png" alt="Clone This Order to New Draft Quote" title="Clone This Order to New Draft Quote" /></a>'.$ifsendtoqb.$ifmarkcompleted.' <a href="/orders/woboxes/'.$order['id'].'" target="_blank"><img src="/img/box-icon.png" width="24" alt="Work Order Boxes" title="Work Order Boxes" /></a>';
				}
	...
	public function editlines($orderid){
		...
		if($thisOrder['status'] == 'Editing'){
    		...
    		if($thisOrderQuote['contact_id'] == '0'){
    			...
    			$customerContact=$this->CustomerContacts->find('all',['conditions'=>['id'=>$thisOrderQuote['contact_id']]])->first()->toArray();
            ...
            $orderLines=$this->OrderItems->find('all',['conditions' => ['order_id' => $orderid]])->toArray();
	...
	public function completeorder($quoteID){
		...
		foreach($orderLookup as $order){
			...
			$thisOrder->cc_qty = $this->getTypeQty($quoteID,'cc');

			$thisOrder->cc_lf = $this->getTypeLF($quoteID,'cc');

			$thisOrder->cc_diff = $this->getTypeDiff($quoteID,'cc');

			$thisOrder->cc_dollar = $this->getTypeDollars($quoteID,'cc');

			$thisOrder->track_lf = $this->getTypeLF($quoteID,'track');

			$thisOrder->track_dollar = $this->getTypeDollars($quoteID,'track');

			$thisOrder->bs_qty = $this->getTypeQty($quoteID,'bs');

			$thisOrder->bs_diff = $this->getTypeDiff($quoteID,'bs');

			$thisOrder->bs_dollar = $this->getTypeDollars($quoteID,'bs');

			$thisOrder->drape_qty = $this->getTypeQty($quoteID,'drapes');

			$thisOrder->drape_widths = $this->getTypeWidths($quoteID,'drapes');

			$thisOrder->drape_diff = $this->getTypeDiff($quoteID,'drapes');

			$thisOrder->drape_dollar = $this->getTypeDollars($quoteID,'drapes');

			$thisOrder->tt_qty = $this->getTypeQty($quoteID,'tt');

			$thisOrder->tt_lf = $this->getTypeLF($quoteID,'tt');

			$thisOrder->tt_diff = $this->getTypeDiff($quoteID,'tt');

			$thisOrder->val_dollar = $this->getTypeDollars($quoteID,'val');

			$thisOrder->corn_dollar = $this->getTypeDollars($quoteID,'corn');

			$thisOrder->wt_hw_qty = $this->getTypeQty($quoteID,'wtht');

			$thisOrder->wt_hw_dollar = $this->getTypeDollars($quoteID,'wtht');

			$thisOrder->blinds_qty = $this->getTypeQty($quoteID,'blinds');

			$thisOrder->blinds_dollar = $this->getTypeDollars($quoteID,'blinds');
			
			$thisOrder->swtmisc_qty = $this->getTypeQty($quoteID,'swtmisc');

			$thisOrder->catchall_qty = $this->getTypeQty($quoteID,'catchall');
...
//			$lineItemMetas=$this->QuoteLineItemMeta->find('all',['conditions'=>['quote_item_id'=>$lineitem['id']]])->toArray();
	...
	public function buildbompdf($orderID){
		...
		$lineItems=$this->WorkLineItems->find('all',['conditions'=>['quote_id'=>$thisOrder['quote_id']],'order'=>['sortorder'=>'asc']])->toArray();
...
//			$lineItemMetas=$this->QuoteLineItemMeta->find('all',['conditions'=>['quote_item_id'=>$lineitem['id']]])->toArray();
		...
		if($this->request->data){
		    ...
		    if(count($boxLookup) > 0){
    			...
    			if($batchTable->delete($thisBatch)){
    				...
    				$orderitemstatustable=TableRegistry::get('OrderItemStatus');
    				$allItemStatuses=$this->OrderItemStatus->find('all',['conditions'=>['sherry_batch_id' => $batchID]])->toArray();
		    ...
		    if(count($boxLookup) > 0){
    			...
    			$quoteLineItems=$this->QuoteLineItems->find('all',['conditions'=>['quote_id'=>$thisQuote['id']]])->toArray();
    			$orderLineItems=$this->OrderItems->find('all',['conditions'=>['order_id'=>$thisOrder['id']]])->toArray();
		...
		if($this->request->data){
			...
			foreach($lineItems as $lineitem){
				...
				if($this->request->data["useline_".$lineitem['id']] == "1"){
					...
					$scheduleTable=TableRegistry::get('OrderItemStatus');
			...
			foreach($lineItems as $lineitem){
				...
				//find all previously shipped items in this line
				$scheduledItemsLookup=$this->OrderItemStatus->find('all',['conditions'=>['order_line_number'=>$lineitem['line_number'],'order_item_id'=>$orderItemID,'work_order_id'=>$orderID,'status IN' => ['Scheduled']]]);
	...
	public function vieworderschedule($orderID){
		...
		$orderLines=$this->OrderItems->find('all',['conditions'=>['order_id'=>$orderID,'quote_id'=>$thisOrder['quote_id']]])->toArray();
		$quoteLines=$this->QuoteLineItems->find('all',['conditions'=>['quote_id'=>$thisOrder['quote_id']],'order'=>['sortorder'=>'asc']])->toArray();
		...
		foreach($quoteLines as $quoteLineData){
			...
			$lineItemMetas=$this->QuoteLineItemMeta->find('all',['conditions'=>['quote_item_id' => $quoteLineData['id']]])->toArray();
			...
			$lineNotes=$this->QuoteLineItemNotes->find('all',['conditions'=>['quote_item_id' => $quoteLineData['id']],'order'=>['time'=>'asc']])->toArray();
			if(count($lineNotes) >0){
				$quoteLinesData[$quoteLineData['id']]['linenotes']=$lineNotes;
			}
		...
		$this->set('allUnscheduledItems',$this->getorderunscheduleditems($orderID));
        ...
        /**PPSASCRUM-3 end **/
			...
			foreach($orderItems as $orderLineItem){
				...
				if($thisQuoteItem['parent_line'] == 0){
					...
					}elseif($thisQuoteItem['product_type'] == 'newcatchall-drapery' || ($thisQuoteItem['calculator_used'] == 'pinch-pleated' || $thisQuoteItem['calculator_used'] == 'pinch-pleated-new') ){
...
/**PPSASCRUM-3 start **/
	...
	public function editschedule($batchID){
		...
		if($this->request->data){		
    		...
    		$checkBatchItems=$this->OrderItemStatus->find('all',['conditions' => ['status' => 'Scheduled', 'sherry_batch_id' => $batchID]])->toArray();
    		
    		$usedLines=array();
    		foreach($checkBatchItems as $batchContents){
    		    
    		    $thisOrderItem=$this->OrderItems->get($batchContents['order_item_id'])->toArray();
    		    
    		    
    		    $usedLines[]=$thisOrderItem['quote_line_item_id'];
    		    if(isset($this->request->data["qty_line_".$thisOrderItem['quote_line_item_id']]) && intval($this->request->data["qty_line_".$thisOrderItem['quote_line_item_id']]) > 0 ){
    		...
    		foreach($this->request->data as $key => $val){
    		    if(substr($key,0,9) == "qty_line_"){
    				...
    				if(!in_array($lineID,$usedLines) && intval($val) > 0){
    				    $thisLineItemData=$this->QuoteLineItems->get($lineID)->toArray();
			...
			if($this->request->data['dateselection'] != $ymdDate){
				...
				$this->logActivity($_SERVER['REQUEST_URI'],'Changed Scheduled Date on Batch #'.$batchID.' from '.$correctedOLDDate.' to '.$correctedNEWDate.' for Order #'.$thisOrder['order_number']);
			}

			$sherryTable=TableRegistry::get('OrderItemStatus');
			
			foreach($this->request->data as $key => $val){
				if(substr($key,0,9) == "qty_line_"){
					
					$thisQuoteLineID=str_replace("qty_line_","",$key);
					$lookupOrderItem=$this->OrderItems->find('all',['conditions'=>['quote_line_item_id' => $thisQuoteLineID]])->toArray();
					foreach($lookupOrderItem as $orderItemRow){
						$thisOrderItemID=$orderItemRow['id'];
					}

					$thisQuoteItem=$this->QuoteLineItems->get($thisQuoteLineID)->toArray();
					...
					$scheduleCheck=$this->OrderItemStatus->find('all',['conditions' => ['sherry_batch_id' => $batchID, 'status' => 'Scheduled', 'order_item_id' => $thisOrderItemID, 'order_line_number' => $thisQuoteItem['line_number']]])->toArray();
			...
			foreach($lineItems as $lineitem){
				...
				//find all previously shipped items in this line
				$scheduledItemsLookup=$this->OrderItemStatus->find('all',['conditions'=>['order_line_number'=>$lineitem['line_number'],'order_item_id'=>$orderItemID,'work_order_id'=>$orderID,'status' => 'Scheduled']]);
				foreach($scheduledItemsLookup as $scheduledItemRow){
	...
	public function markscheduleditemcompleted($batchID,$frombreakdown=0){
		...
		if($this->request->data){
            ...
            $thisWorkOrder=$this->Orders->get($orderID)->toArray();
            
			$thisOrderDayStatuses=$this->OrderItemStatus->find('all',['conditions'=>['sherry_batch_id' => $batchID, 'status' => 'Scheduled'],'order'=>['order_line_number'=>'asc']])->toArray();
			
			$statusTable=TableRegistry::get('OrderItemStatus');
	...
	public function markscheduleditemshipped($batchID,$frombreakdown=0){
		...
		if($this->request->data){
            ...
            */
    	    ...
    	    $lineItems=$this->QuoteLineItems->find('all',['conditions'=>['quote_id'=>$thisWorkOrder['quote_id']],'order'=>['sortorder'=>'asc']])->toArray();
    		...
    		foreach($lineItems as $lineitem){
    			...
    			//find all previously shipped items in this line
    			$scheduledItemsLookup=$this->OrderItemStatus->find('all',['conditions'=>['order_line_number'=>$lineitem['line_number'],'order_item_id'=>$orderItemID,'work_order_id'=>$thisWorkOrder['id'],'status' => 'Scheduled']]);
    			foreach($scheduledItemsLookup as $scheduledItemRow){
	...
	public function markscheduleditemuninvoiced($batchID,$frombreakdown=0){
        ...
        $lookupItemStatusMarkedInvoicedThisBatch=$this->OrderItemStatus->find('all',['conditions'=>['sherry_batch_id'=>$batchID,'status'=>'Invoiced']])->toArray();
		...
		for($i = 1; $i <= $days; $i++){
			...
			foreach($scheduleEntries as $scheduleItem){
				...
				foreach($batchNotes as $batchNote){
					...
					$thisRowOutput .= "<tr class=\"batchnoterow ".$batchNoteClass." ".$batchnotetypeclass."\"><td colspan=\"2\" class=\"noterowlabel\"><b>".strtoupper($batchNote['note_type'])." NOTE:</b></td><td colspan=\"24\" class=\"notecontent\">".str_replace("'","\'",preg_replace("/\r\n|\r|\n/",'<br/>',$batchNote['message']))."</td></tr>";
				}
	...
	public function voidshipmentbatch($batchID,$frombreakdown=0){
		...
		if($this->request->data){
			...
			$shipmentStatusLines=$this->OrderItemStatus->find('all',['conditions'=>['sherry_batch_id' => $batchID, 'status' => 'Shipped']])->toArray();
	...
	public function voidbatchcompletion($batchID,$frombreakdown=0){
		...
		if($this->request->data){
			...
			$shipmentStatusLines=$this->OrderItemStatus->find('all',['conditions'=>['sherry_batch_id' => $batchID, 'status' => 'Completed']])->toArray();
			foreach($shipmentStatusLines as $shipmentStatus){
				$thisStatus=$orderitemstatustable->get($shipmentStatus['id']);
	...
	public function woboxes($orderID){
	    $thisWO=$this->Orders->get($orderID)->toArray();
	    $this->set('thisWO',$thisWO);
	    
	    $warehouseLocations=$this->WarehouseLocations->find('all')->toArray();
        $this->set('warehouseLocations',$warehouseLocations);
        
        $lineItems=$this->QuoteLineItems->find('all',['conditions'=>['quote_id'=>$thisWO['quote_id']],'order'=>['sortorder'=>'asc']])->toArray();
        $this->set('lineItems',$lineItems);
	...
	public function batchboxes($batchID){
	    $thisBatch=$this->SherryBatches->get($batchID)->toArray();
	    $this->set('thisBatch',$thisBatch);
	    $thisWO=$this->Orders->get($thisBatch['work_order_id'])->toArray();
	    $this->set('thisWO',$thisWO);
		...
		foreach($lineItems as $lineitem){
			...
			//find all previously shipped items in this line
			$scheduledItemsLookup=$this->OrderItemStatus->find('all',['conditions'=>['order_line_number'=>$lineitem['line_number'],'order_item_id'=>$orderItemID,'work_order_id'=>$thisWO['id'],'status' => 'Scheduled']]);
			foreach($scheduledItemsLookup as $scheduledItemRow){
    ...
    public function boxlabel($boxID){
		...
		foreach($lineItems as $lineitem){
			...
			//find all previously shipped items in this line
			$scheduledItemsLookup=$this->OrderItemStatus->find('all',['conditions'=>['order_line_number'=>$lineitem['line_number'],'order_item_id'=>$orderItemID,'work_order_id'=>$thisOrder['id'],'status' => 'Scheduled']]);
			foreach($scheduledItemsLookup as $scheduledItemRow){
        ...
        if($this->request->data){
            ...
            foreach($lineItems as $lineitem){
                ...
                if(isset($this->request->data["boxcontent_lineitem_".$lineitem['id']])){
                    ...
                    if(intval($this->request->data["boxcontent_lineitem_".$lineitem['id']]) == 0){
                        ...
                        $boxEntryCheck=$this->SherryBatchBoxContents->find('all',['conditions' => ['box_id' => $boxID, 'line_number' => $lineitem['line_number'] ] ]);
            ...
            if(!$boxID){
    			...
    			foreach($lineItems as $lineitem){
    				...
    				$lineItemsArray[$lineitem['id']]['this_batch_other_boxes'] = $thisBatchOtherBoxed;
    				$thisBoxContents=$this->SherryBatchBoxContents->find('all',['conditions' => ['box_id' => $thisBox['id'], 'line_number' => $lineitem['line_number'] ]])->toArray();
	...
	public function editbatchdate($batchID){
	    ...
	    if($this->request->data){
	        ...
	        foreach($statusLinesLookup as $statusLine){
	            $statusLineEdit=$this->OrderItemStatus->get($statusLine['id']);
	            $statusLineEdit->time=strtotime($this->request->data['dateselection'].' 18:00:00');
	            $this->OrderItemStatus->save($statusLineEdit);
	        }
	...
	switch($subaction){
		...
		case "add":
			if($this->request->data){
				...
				$newfaciltiy->default_address=$this->request->data['default_address'];
				...
				if($faciltiyTable->save($newfaciltiy)){
					//hooray!
					$this->Flash->success('Successfully added Facility "'.$this->request->data['facility_name']);
		...
		case "edit":
			...
			if($this->request->data){
				...
				$thisfaciltiy->default_address=$this->request->data['default_address'];
...
public function getshiptoslist(){
	...
	$shipTo[]=array(
		...
		'1' => $shipTos['shipping_address_1'],
		'2' => $shipTos['shipping_address_2'],
		'3' => $shipTos['shipping_city'],
		'4' => $shipTos['shipping_state'],
		'5' => $shipTos['shipping_country'],
		'6' => $shipTos['shipping_zipcode'],
		'7' => date('n/j/Y - g:iA',$shipTos['created'])
	);
}
...
public function markshipto($frombreakdown=0){
	...
	if($this->request->data){
		...
		if($frombreakdown == 1){
			...
			echo "parent.$('#default_address').val('".$lastCreated['id']."');";
...
public function  markfacility($frombreakdown=0){
	...
	if($this->request->data){
		...
		$newfacility->default_address=$this->request->data['default_address'];
		...
		if($frombreakdown == 1){
			...
			";
			echo "parent.$('#allfacility').html('');";
			echo "parent.$.each({".$options."}, function(key, value) {
				parent.$('#allfacility').append(parent.$('<option></option>')
				.attr(\"value\", value).text(key));
			});";
			echo " parent.$('#allfacility').val('".$lastCreated['id']."');";
			...
			//echo "parent.$('#userAddressesSelection').val('default');";
			 echo "parent.$('input[type=radio][name=userAddressesSelection][value=\"default\"]').prop('checked', true); parent.$('#userAddressesSelection').val('default');parent.$('input[type=radio][name=userAddressesSelection]').change();";
...
public function getFacilityDetails($facilityId){
	if(!empty($facilityId)) {
		...
		echo $fac['facility_code'] . ":" . $fac['attention'];
	}
	exit;
}
...
public function getFacilityAddressDetails($facilityId){
	if(!empty($facilityId)) {
		...
		echo $shipto['shipping_address_1'].":".$shipto['shipping_address_2'] .":".$shipto['shipping_city'].":".$shipto['shipping_state'].":".$shipto['shipping_country'].":".$shipto['shipping_zipcode'].":".$shipto['attention'];
	}
	exit;
}
...
public function getShipTodetails($shipsearch,$feild ='shipping_address_1') {
	...
	echo $shipto['shipping_address_1'].":".$shipto['shipping_address_2'] .":".$shipto['shipping_city'].":".$shipto['shipping_state'].":".$shipto['shipping_country'].":".$shipto['shipping_zipcode'].":".$shipto['attention'].":".$shipto['id'];
...
public function getCustomerAddressDetails($id){
	...
	echo $shipto['shipping_address'].":".'' .":".$shipto['shipping_address_city'].":".$shipto['shipping_address_state'].":".$shipto['shipping_address_country'].":".$shipto['shipping_address_zipcode'].":".'';
	exit;
}
...
public function  getFacilityByCode($id){
	...
	echo $facility['id'].":".''.$facility['facility_code'].":".$facility['attention'];
	exit;
}

public function searchFacilityfield($type,$q){
	$this->autoRender=false;
	$facilityFind=$this->Facility->find('all',['conditions' => [ 'facility_name LIKE' => '%'.$q.'%'],'order'=>['facility_name'=>'asc']])->group('facility_name')->toArray();

	
	
	$out=array();
	foreach($facilityFind as $facility){
		$out[$facility['id']]=$facility['facility_name'];
	}
		...
		foreach($getOrders as $order){
			...
			if($order['status'] == 'Needs Line Items'){
				...
				$orders[]=array(
					...
					'0' => '<a href="/orders/editlines/'.$order['id'].'"/><img src="/img/vieworder.png" alt="Line Items" title="Line Items" /></a><a href="/orders/woedit/'.$order['id'].'"><img src="/img/edit.png" alt="Edit Order Details" title="Edit Order Details" /></a>  <a href="/orders/woboxes/'.$order['id'].'" target="_blank"><img src="/img/box-icon.png" width="24" alt="Work Order Boxes" title="Work Order Boxes" /></a>',
...
<table width=\"100%\" cellpadding=\"3\" border=\"1\" bordercolor=\"#000000\" cellspacing=\"0\" align=\"right\">
	...
	public function changelinetally($lineid,$newvalue, $mode='order'){
		...
		//$lineTable=TableRegistry::get('OrderItems');

		//$thisLine=$lineTable->get($lineid);
		echo $lineid;
		$thisLine=$this->OrderItems->find('all', ['conditions' => ['id'=>$lineid]])->first()->toArray() ;
	...
	public function woeditlines($orderid){
		...
		if($thisOrder['status'] == 'Editing'){
    		...
    		$availableProjects=$this->Projects->find('all',['conditions'=>['customer_id' => $thisOrderQuote['customer_id']]])->toArray();
    		$this->set('availableProjects',$availableProjects);
    	echo $thisOrderQuote['contact_id']."   fff";
    		if($thisOrderQuote['contact_id'] == '0'){
    			$customerContact=false;
    		}else{
	...
	public function rtmalllines($orderID,$newval){
	    $thisOrder=$this->WorkOrders->get($orderID)->toArray();
	    
	    
	    $allLines=$this->WorkOrderItems->find('all',['conditions' => ['order_id' => $orderID]])->toArray();
	    foreach($allLines as $line){
	        $thisOrderItem=$this->WorkOrderItems->get($line['id']);
	        $thisOrderItem->released_to_manufacture=$newval;
	        $this->WorkOrderItems->save($thisOrderItem);
	    }
	    
	    if($newval==0){
	        $this->logActivity($_SERVER['REQUEST_URI'],'Unreleased All Lines of WorkOrder# '.$thisOrder['order_number'].' from Manufacture');
	    }else{
	        $this->logActivity($_SERVER['REQUEST_URI'],'Released All Lines of WorkOrder# '.$thisOrder['order_number'].' to Manufacture');
	    }
	    
	    return $this->redirect('/orders/woview/'.$orderID);
	}
