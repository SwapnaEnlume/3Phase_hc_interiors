...
if(count($priceListResults) == 0){
	...
	foreach($priceListResults as $productid => $CCproduct){
		echo "<fieldset>";

		echo "<legend>".$CCproduct['title'].", <em>Colors: <u>";
		$colorOut='';
		...
		foreach($CCproduct['sizes'] as $num => $size){
			...
			foreach($allCCSizes as $sizeRow){
				if($sizeRow['id'] == $size['size_id']){
					echo "<div class=\"sizeEntry\"><label>".$sizeRow['title']." Price:</label> $<input type=\"number\" step=\"any\" name=\"cc_".$productid."_size_".$sizeRow['id']."_price\" placeholder=\"".$size['price']."\" /></label></div>";
				}
			}
		}
