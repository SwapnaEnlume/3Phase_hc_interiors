...
class ReportsController extends AppController
{
	...
	public function backlogreport($type='backlog'){
	    $productMap=array(
            ...
            /* PPSASCRUM-384: start */
            'accordiafold-drapery' => 'Accordiafold Drapery'
            /* PPSASCRUM-384: end */
            /* PPSASCRUM-305: end */
			/* PPSASCRUM-326: end */
	    ...
	    if($this->request->data){
			...
			foreach($orderLookup as $orderRow){
				...
				foreach($quoteItemsLoop as $quoteItem){
					...
					foreach($orderItemFind as $orderItem){
    						...
    						/* PPSASCRUM-326: start */
    						$remainingUnscheduled=(intval($orderItem['qty']) - $qtyScheduled);
    						$remainingUninvoiced=(intval($orderItem['qty']) - $qtyInvoiced);
    						$remainingUnshipped = (intval($orderItem['qty']) - $qtyShipped);
    						$remainingIncompleteScheduled= (intval($orderItem['qty']) - $qtyCompleted);
    						/* PPSASCRUM-326: end */
    						...
    						*/
    						...
    						if($metaArray['lineitemtype']=='calculator'){
                                ...
                                } /* PPSASCRUM-326: start */ /* PPSASCRUM-384: start */ elseif (
                                    $metaArray['calculator-used'] == 'pinch-pleated' || $metaArray['calculator-used'] == 'new-pinch-pleated' ||
    						...
    						if($quoteItem['product_type'] == 'track_systems'){
                                ...
                                } /* PPSASCRUM-384: start */ elseif ($metaArray['calculator-used'] == 'pinch-pleated' || $quoteItem['product_type'] == 'newcatchall-drapery' ||
                                          $metaArray['calculator-used'] == 'ripplefold-drapery' || $metaArray['calculator-used'] == 'accordiafold-drapery') /* PPSASCRUM-384: end */ {
    								if($metaArray['unit-of-measure'] == 'pair'){
    									if(isset($metaArray['rod-width']) && strlen(trim($metaArray['rod-width'])) >0){
    										$cutwidthValue .= $metaArray['rod-width']." (Rod)";
    									}
    						...
    						if($loop['unbatched'] > 0)
    							...
    							if($metaArray['lineitemtype'] == 'simpleproduct'){
    								...
    								if(floatval($metaArray['total-yds']) >0){
    								    /* PPSASCRUM-326: start */
    									$totalydsvalue .= round(((floatval($metaArray['total-yds']) / floatval($orderItem['qty'])) * intval($qtyValue)),2);
    									/* PPSASCRUM-326: end */
    								}else{
    									$totalydsvalue .= round((floatval($metaArray['yds-per-unit']) * floatval($qtyValue)),2);
    								}
    						...
    						if(!is_null($orderRow['type_id']) && $orderRow['type_id'] > 0){
    							...
    							/* PPSASCRUM-326: start */
    							$objPHPExcel->getActiveSheet()->SetCellValue('Z'.$rowCount, $cubicleCurtainLFValue); // CCLF
			...
			/* PPSASCRUM-326: start */
			/* use this for local generation of Excel backlog report */
