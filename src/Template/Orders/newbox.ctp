<h2 style="text-align:center;">Create New Box</h2>
<?php
...
<tr>
<td width=\"20%\">";
echo $this->Form->input('length',['label' => 'Length (in)','type'=>'number','required'=>true,'autocomplete'=>'off']);
echo "</td>
<td width=\"20%\">";
echo $this->Form->input('width',['label' => 'Width (in)','type'=>'number','required'=>true,'autocomplete'=>'off']);
echo "</td>
<td width=\"20%\">";
echo $this->Form->input('height',['label' => 'Height (in)','type'=>'number','required'=>true,'autocomplete'=>'off']);
echo "</td>
<td width=\"20%\">";
echo $this->Form->input('weight',['label' => 'Weight (lbs)','type'=>'number','required'=>true,'autocomplete'=>'off','step'=>'0.01']);
echo "</td>
<td width=\"20%\">";
...
<thead>
<tr>
<th class=\"linenumber\">WO Line#</th>
...
foreach($lineItems as $idnum => $line){
    if($line['data']->enable_tally == 1 && intval($line['this_batch']) > 0){
        ...
        <td class=\"boxqty\"><button type=\"button\" onclick=\"subtractOne('".$line['data']->id."')\">-</button><input type=\"number\" name=\"boxcontent_lineitem_".$line['data']->id."\" autocomplete=\"off\" min=\"0\" max=\"".$line['this_batch']."\" value=\"0\" /><button type=\"button\" onclick=\"addOne('".$line['data']->id."')\">+</button></td>
...
echo $this->Form->submit('Create Box of 0 Items');
