...
if($mode=='edit'){
    ...
    echo "<h3>Edit ".$typeTitle." Cubicle Curtain Catch-All Line Item</h3><hr>";
}else{
    echo "<h3>Add Cubicle Curtain Catch-All Line to Quote</h3><hr>";
}
...
echo $this->Form->label('Cubicle Curtain Sub-Class');
...
echo $this->Form->select('unit_type',['dozen'=>'Dozen','each'=>'Each','lot'=>'Lot','package'=>'Package','pair'=>'Pair','set'=>'Set','yards'=>'Yards','box'=>'Box'],['disabled'=>true,'value'=>$thisUnitVal]);
else
...
// PPSASCRUM-160 start:
$momcheckboxArr=['label'=>'MOM Fabric Special Cost p/yd','type'=>'checkbox','value'=>1];
// PPSASCRUM-160 end
...
<td width=\"50%\" valign=\"top\" style=\"padding:15px;\">
    ...
    //Desired Finished Width field *required
    if(isset($lineitemmeta['width']) && is_numeric($lineitemmeta['width'])){
    	$widthval=$lineitemmeta['width'];
    }else{
    	$widthval='';
    }
    echo $this->Form->input('width',['type'=>'number','min'=>'0','value'=>$widthval,'label'=>'Desired Finished Width', 'required'=>true]);


    
    //cut width field *optional
    if(isset($lineitemmeta['cut-width']) && is_numeric($lineitemmeta['cut-width'])){
    	$cutwidthval=$lineitemmeta['cut-width'];
    }else{
    	$cutwidthval='';
    }
    echo $this->Form->input('cut-width',['type'=>'number','min'=>'0','value'=>$cutwidthval,'label'=>'CUT WIDTH']);
    ...
    echo $this->Form->input('length',['type'=>'number','min'=>'0','value'=>$lengthval,'label'=>'FINISH LENGTH','required'=>true]);

    
    echo "<h5>Mesh</h5>";
    
    
    //mesh size field *optional
    if(isset($lineitemmeta['mesh']) && is_numeric($lineitemmeta['mesh'])){
    	$meshval=$lineitemmeta['mesh'];
    }else{
    ...
    //finished mesh field *autocalc
    if(isset($lineitemmeta['finished-mesh']) && is_numeric($lineitemmeta['finished-mesh'])){
    	$fmeshval=$lineitemmeta['finished-mesh'];
    }else{
    ...
    //mesh color field *optional
    if(isset($lineitemmeta['mesh-color']) && strlen(trim($lineitemmeta['mesh-color'])) >0){
    	$meshcolorval=$lineitemmeta['mesh-color'];
    }else{
    	$meshcolorval='';
    }
    echo $this->Form->input('mesh-color',['label'=>'Mesh Color','value'=>$meshcolorval]);
    ...
    <td width=\"50%\" valign=\"top\" bgcolor=\"#EEEEEE\" style=\"padding:15px;\">
        ...
        //widths each field *optional
        if(isset($lineitemmeta['widths-each']) && strlen(trim($lineitemmeta['widths-each'])) > 0){
        	$widthseachval=$lineitemmeta['widths-each'];
        }else{
        	$widthseachval='';
        }
        echo $this->Form->input('widths-each',['label'=>'Widths Each','value'=>$widthseachval]);
        ...
        echo $this->Form->input('labor-billable',['type'=>'number','step'=>1,'label'=>'Labor Billable LF','required'=>true,'value'=>$laborbillableval]);
        ...
        echo $this->Form->input('yds-per-unit',['label'=>'Yards per unit','value'=>$ydspbsval, 'required'=>true]);
...
function checkSubmittable(){
    ...
    if(parseFloat($('#mesh').val()) > 0.00){
        $('#finished-mesh').val( (parseFloat($('#mesh').val()) + ".$settings['mesh_heading'].") );
    }else{
        $('#finished-mesh').val( 0 );
    }
    ...
    if($('#qty').val() != '' && $('#widths-each').val() != ''){
        $('#total-widths').val( ( parseFloat($('#widths-each').val()) * parseInt($('#qty').val()) ).toFixed(2) );
    }else{
        $('#total-widths').val('');
    }
    ...
    */
    ...
    if(missingCount >0){
        ...
        if( $('.myForm').attr('action').length !=0)
         $('div.submit input[type=submit]').prop('disabled',false);
    }
    /*PPSA-33 end */
