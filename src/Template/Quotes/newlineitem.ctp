...
switch($type){
	case "simple":
		...
		echo "<input type=\"hidden\" name=\"ordermode\" id=\"ordermode\" value=\"".$ordermode."\" />";
...
if($quoteData['type_id']==1)
    echo $this->Form->input('location',['label'=>'Location','required'=>true]);
else
    echo "<p><label for=\"location\">Location</label> <input type=\"text\" name=\"location\" id=\"location\" value=''></p>";
		...
		echo "<div id=\"databox\"></div>";
            ...
            if($ordermode == 'workorder'){
                ...
                echo $this->Form->select('so_line_number',$soLineNumberLists,['required'=>false,'empty'=>'--Select SO Line Number--','value'=>'0', 'id'=>'so_line_number', 'default'=>'0' ]);
            }else{
                		echo "<input type=\"hidden\" name=\"so_line_number\" id=\"so_line_number\" value=\" \" />";
		...
		$(function(){
			...
			$('#addsimpleproducttoquote').click(function(){
                 ...
                 var btn = document.getElementById('addsimpleproducttoquote');
                 btn.disabled = true;
...
";
echo "if(($('#quotetype').val() == 1)  && $('#location').val() == ''){ $('#location').addClass('stillneeded');
...
if(isset($subaction) && $subaction == 'cubicle-curtain'){
 ...
 echo "if($('input[name=so_line_number]').val()) 
               $('input[name=so_line_number]').val(0);
               ";
echo "
				$.get('/quotes/";
		...
		if(isset($subaction) && $subaction == 'cubicle-curtain'){
			...
			echo "addcctoquote/".$quoteID."/'+$('#fabricid').val()+'/'+$('#fabricidwithcolor').val()+'/'+$('#ccid').val()+'/'+$('#sizeid').val()+'/'+$('#qtyvalue').val()+'/'+$('input[name=so_line_number]').val()+'/'+parseFloat($('#priceeachvalue').html().replace('$',''))+'/'+$('#length').val()+'/'+$('#meshoption').val()+'/'+$('#weight').val()+'/'+$('#yards').val()+'/'+$('#labor-lf').val()+'/'+$('#difficulty').val()+'/'+$('#specialpricing').val()+'/'+usefinishwidthorno+'/'+usealiasyesno+'/'+escapeTextFieldforURL($('#location').val() )+'/'+'".$ordermode."'";
			...
			echo "addbstoquote/".$quoteID."/'+$('#fabricid').val()+'/'+$('#fabricidwithcolor').val()+'/'+$('select#bssize option:selected').attr('data-bsid')+'/'+$('#sizeid').val()+'/'+$('#qtyvalue').val()+'/'+$('input[name=so_line_number]').val()+'/'+$('#price').val()+'/'+$('#weight').val()+'/'+$('#difficulty').val()+'/'+$('#yards').val()+'/'+isquilted+'/'+$('#mattresssize').val()+'/'+$('#topwidthsl').val()+'/'+$('#dropwidthsl').val()+'/'+$('#quiltpattern').val()+'/'+$('#topcutsw').val()+'/'+$('#dropcutsw').val()+'/'+$('#repeat').val()+'/'+usealiasyesno+'/'+specialpricingyesno+'/'+$('#layout').val()+'/'+escapeTextFieldforURL($('#location').val() )+'/'+'".$ordermode."'";
			
			

		}elseif(isset($subaction) && $subaction == 'services'){
			echo "addservicetoquote/".$quoteID."/'+$('#serviceid').val()+'/'+$('#priceinput').val()+'/'+$('#qtyvalue').val()+'/'+$('input[name=so_line_number]').val()+'/0/0/'+escapeTextFieldforURL($('#location').val())+'/'+'".$ordermode."'";
		}elseif(isset($subaction) && $subaction=='window-treatment'){
			
			echo "addwttoquote/".$quoteID."/'+$('#wttype').val()+'/'+$('#fabricid').val()+'/'+$('#fabricidwithcolor').val()+'/'+$('#wtid').val()+'/'+$('#sizeid').val()+'/'+$('#qtyvalue').val()+'/'+$('input[name=so_line_number]').val()+'/'+$('#price').val()+'/'+$('#weight').val()+'/'+$('#difficulty').val()+'/'+$('#yards').val()+'/'+$('#haswelts').val()+'/'+$('#liningoption').val()+'/'+$('#pairorpanel').val()+'/'+$('#labor-lf').val()+'/'+$('#specialpricing').val()+'/'+$('#length').val()+'/'+usealiasyesno+'/'+shortpointval+'/'+escapeTextFieldforURL($('#location').val() )+'/'+'".$ordermode."'";
			...
			/**PPSASCRUM-27 start **///$('#so_line_number').val('');$( "#so_line_number option:selected" ).text();

           	echo "addtracktoquote/".$quoteID."/'+$('select[name=system_id]').val()+'/'+$('#qtyvalue').val()+'/'+$('input[name=so_line_number]').val()+'/'+$('select[name=system_id] option:selected').attr('data-price')+'/'+escapeTextFieldforURL($('#description').val())+'/'+escapeTextFieldforURL($('#location').val() )+'/'+$('#ordermode').val()";
			//addtracktoquote($quoteID,$trackItemID,$qty,$price)
			/**PPSASCRUM-27 end **/
		}
		...
		echo ",function(response){
			if(response=='OK'){
				/* PPSASCRUM-139: start */
				location.replace('/quotes/add/".$quoteID."/".$ordermode."?price_list=created_success');
				/* PPSASCRUM-139: end */
			}
		});
...
<fieldset id=\"calculatorfabric\"><legend>Fabric Selection</legend>";
                    ...
                    $(function(){
                        ...
                        $('input[name=fabric_name]').autocomplete({
                           source: function( request, response){
                               $.ajax({
                                  ...
                                  success: function(data){
								  	/* PPSASCRUM-100: start */
									  fabricVendorMap = data;
									  console.log('This is fabricVendorMap: ' + JSON.stringify(fabricVendorMap));
									  for (obj in data) {
										let fabricVendorName = data[obj].split(';;');
										data[obj] = fabricVendorName[0] + ' '.repeat(125 - fabricVendorName[0].length) + fabricVendorName[1];
									  }
									/* PPSASCRUM-100: end */										
									response(data);
                                  }
                           ...
                           select: function(event,ui){
							   ...
							   /* PPSASCRUM-100: start */
							   var fabricVendorSelected = ui.item.value;
							   var fabricVendorId = Object.keys(fabricVendorMap)[Object.values(fabricVendorMap).findIndex(ele => ele == fabricVendorSelected)];
							   var vendorId = fabricVendorId.split(';;')[1].trim();
							   var fabricName = fabricVendorSelected.split(/\s(?=\[)/gm)[0].trim();
							   console.log('Fabric id: ' + fabricVendorId.split(';;')[0]);
							   console.log('Vendor id: ' + fabricVendorId.split(';;')[1]);
							   console.log('Fabric Vendor names: ' + fabricVendorSelected);
							   console.log('Fabric name: ' + fabricName);
                               $.get('/quotes/getfabriccolorlist/'+encodeURIComponent(fabricName)+'/'+".$quoteID."+';;'+vendorId,function(response){
						   ...
						   .autocomplete(\"instance\")._renderItem = function(ul, item) {
								...
								divElement.id = \"ui-id-\" + divTagIndex++;
								divElement.tabIndex = -1;
								divElement.className = 'ui-menu-item-wrapper';
								divElement.style.cssText = 'display: flex; justify-content: space-around; padding: 5px;';

								let fabricVendorName = item.value.split(/\s(?=\[)/gm);

								// Create the first <span> element
								const span1 = document.createElement('span');
								span1.style.flex = '1';
								span1.textContent = fabricVendorName[0].trim();

								console.log('Fabric name: ', span1.textContent);

								// Create the second <span> element
								const span2 = document.createElement('span');
								span2.style.cssText = 'flex: 1; white-space: nowrap; justify-content: flex-start;';
								span2.textContent = fabricVendorName[1];

								console.log('Vendor name: ', span2.textContent);
								...
								return $(\"<li></li>\")
									.data(\"item.autocomplete\", divElement)
									.html(divElement) // Use the HTML string directly
									.appendTo(ul);
							};
							/* PPSASCRUM-100: end */
                    });
				...
				function doconfirmaliascontinue(fabricid){
					...
					location.replace('/quotes/newlineitem/".$quoteID.'/'.$ordermode."/calculator/".$subaction."/'+fabricid+'/'+usealias);
				}

				function doselectfabriccolor(fabricid,hasalias){
					
					if(hasalias==0){
						location.replace('/quotes/newlineitem/".$quoteID.'/'.$ordermode."/calculator/".$subaction."/'+fabricid);
					}else{
						$.fancybox({
							'type':'html',
							'width':500,
	...
	case "custom":
		...
		if($lineitemdata['product_type'] == 'track_systems'|| $lineitemdata['product_subclass'] == '3'){
			...
			//echo $this->Form->input('fabric_profit',['label'=>'Fabric Profit','autocomplete'=>'off','placeholder'=>'If Applicable','required'=>false]);
				...
				/**PPSASCRUM-201 start **/
				//echo $this->Form->input('room_number',['label'=>'Location / Room Number','autocomplete'=>'off','placeholder'=>'If Applicable','required'=>true]);
		
            if($quoteData['type_id'] ==1)
			   echo $this->Form->input('room_number',['label'=>'Location / Room Number','placeholder'=>'If Applicable','required'=>true]);
			else
			echo $this->Form->input('room_number',['label'=>'Location / Room Number','placeholder'=>'If Applicable','required'=>false]);
/**PPSASCRUM-201 end **/
...
$(function(){
	...
	$('#cancelbutton').click(function(){
	    ...
	    if ('<?php echo $ordermode; ?>' == 'workorder') {
	        location.replace('/orders/editlines/<?php echo $quoteData['order_id']; ?>/workorder')
	    } else {
...
/* PPSASCRUM-100: start */
.autocomplete-items { position: absolute; border: 1px solid #d4d4d4; border-bottom: none; border-top: none; z-index: 99; top: 100%; left: 0; right: 0; display: flex; flex-direction: column; }

.autocomplete-item { display: flex; justify-content: space-around; padding: 5px; }

.fabric-name { flex: 1; }

.vendor-name { white-space: nowrap; flex: 1; justify-content: flex-start; }
/* PPSASCRUM-100: end */
</style>