...
/*PPSA-33 end*/
/*PPSASCRUM-90: start*/
div.addaslineitembutton button[type=submit]:disabled,
div.addaslineitembutton button[type=submit][disabled]{ cursor:not-allowed !important; background:#444 !important; color:#CCC !important; }
/*PPSASCRUM-90: end*/
</style>
<script>
function changefabricmodal(){
    $.fancybox({
        ...
        'href':'/quotes/changecalcitemfabric/'+$('#fabricid').val()+'/box-pleated-valance',
		/* PPSASCRUM-100: end */
        'autoSize':false,
        /* PPSASCRUM-100: start */
        /* 'width':450, */
...
if(intval($quoteID) >0){
	...
	if($isEdit){
        ...
        /* PPSASCRUM-317: start */
        // echo $this->Form->button('Save Changes',['type'=>'submit','onClick'=>'return canCalculate();' ,'class'=>'SubmitButton']);
		...
		/* PPSASCRUM-317: start */
// 		echo $this->Form->button('Add To Quote',['type'=>'submit','onClick'=>'return canCalculate();' ,'class'=>'SubmitButton']);
...
<!-- changes for PPSASCRUM-90: start -->
<div id="hingeswarningbox"></div>
<!-- changes for PPSASCRUM-90: end -->
<style>
...
/* changes for PPSASCRUM-90: start */
#hingeswarningbox { display:none; position: fixed; padding:5px; bottom:125px; right:20px; width:510px; background:#FFCFBF; color:red; font-weight:bold; border:3px solid red; z-index:6667; font-size:12px; }
/* changes for PPSASCRUM-90: end */
</style>
...
if(qty >0 && $('select[name=fabric-cost-per-yard]').val() != '' && tot_yds > 0){
		...
		/*PPSASCRUM-151: start*/
        if (heightShortErrors == 0 && heightLongErrors == 0) {
            $('div.addaslineitembutton button[type=submit]').removeAttr('disabled');
        } else {
            $('div.addaslineitembutton button[type=submit]').attr('disabled',true);
        }
        /*PPSASCRUM-151: end*/
        ...
        if(warncount == 0){
             ...
             /*PPSASCRUM-90: start*/
             $('#warningbox').html(warningboxcontent).css("display", "block").show('fast');
             /*PPSASCRUM-90: end*/
        }
...
$(function(){
    $('input[name=hingecount]').change(function() {
        /*PPSASCRUM-90: start*/
        canCalculate();
        /*PPSASCRUM-90: end*/
    });
...
function canCalculate(){
	...
	*/
    ...
    if($('#hinged').is(':checked')){
        ...
        if ($('input[name=hingecount]').val() < 1) {
            $('input[name=hingecount]').removeClass('notvalid').removeClass('validated').addClass('notvalid');
            $('input[name=hingecount]').parent().addClass('badvalue');
            $('#hingeswarningbox').html('<span id="lower_hinges_count_msg"><img src="/img/delete.png" /> Hinge count should be greater than 0<br>').css("display", "block").show();
            errorcount++;
        } else {
            $('input[name=hingecount]').removeClass('notvalid').removeClass('validated').addClass('validated');
            $('input[name=hingecount]').parent().removeClass('badvalue');
            $('#hingeswarningbox').hide('fast');
        }
    }else{
        $('#hingecountwrap').hide('fast');
        $('#hingecount').removeClass('notvalid').removeClass('validated').addClass('validated');
        $('#hingecount').parent().removeClass('badvalue');
        $('#hingeswarningbox').hide('fast');
        <?php if ( isset($thisItemMeta['hinged']) && $thisItemMeta['hinged']=='1') {
            if (isset($thisItemMeta['hingecount']) && is_numeric($thisItemMeta['hingecount'])) { ?>
                if ($('#hingecount').val() == 0) {
                    $('#hingecount').val('<?php echo $thisItemMeta['hingecount'] ?>');
                }
                else {
                    $('#hingecount').val($('#hingecount').val());
                }
            <?php }
            } else { ?>
                if ($('#hingecount').val() == 0) {
