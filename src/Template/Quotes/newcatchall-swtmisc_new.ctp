...
switch($mode){
    ...
    break;
    case 'workorderedit':
        echo "<h3>Edit <u>WORK ORDER</u> SWT Miscellaneous Catch-All Line Item</h3>";
    break;
    default:
...
if($mode=='workorderedit' && isset($lineitemmeta['line_item_title'])){
    $thisLineItemTitle=$lineitemmeta['line_item_title'];
}else{
    $thisLineItemTitle=$thisLineItem['title'];
}

echo $this->Form->input('line_item_title',['label'=>'Line Item Title','required'=>true,'autocomplete'=>'off','value'=>$thisLineItemTitle]);

if($mode=='workorderedit' && isset($lineitemmeta['description'])){
    $thisLineItemDescription=$lineitemmeta['description'];
}else{
    $thisLineItemDescription=$thisLineItem['description'];
}

echo $this->Form->input('description',['required'=>false,'maxlength'=>300,'label'=>'Description (300 characters max)','value'=>$thisLineItemDescription]);
...
if($mode=='workorderedit'){
    $qtyReadOnly=true;
    $qtyCSSClassAdd=' readonlywrap';
}else{
    $qtyReadOnly=false;
    $qtyCSSClassAdd='';
}
echo "<div class=\"".$qtyCSSClassAdd."\">";
echo $this->Form->input('qty',['type'=>'number','label'=>'Qty','required'=>true,'autocomplete'=>'off', 'min' => '1','value'=>$thisLineItem['qty'],'readonly'=>$qtyReadOnly]);
...
echo $this->Form->select('unit_type',['dozen'=>'Dozen','each'=>'Each','lot'=>'Lot','package'=>'Package','pair'=>'Pair','set'=>'Set','yards'=>'Yards'],['required'=>true,'value'=>$thisUnitVal,'disabled'=>$qtyReadOnly,'readonly'=>$qtyReadOnly]);
echo "</div>";

echo $this->Form->input('price',['label'=>'Price Per Unit','required'=>true,'autocomplete'=>'off','value'=>$thisLineItem['best_price'],'readonly'=>$qtyReadOnly]);
echo "</div>"; //end QTY+UNIT+PRICE wrapper


echo "<fieldset id=\"lineitemimagewrap\"><legend>Line Item Image</legend>";

if(($mode=='edit' || $mode=='workorderedit') && $lineitemmeta['image_method']=='upload'){
    $thisMethod='library';
}else{
...
if(($mode=='edit' || $mode=='workorderedit') && ($lineitemmeta['image_method'] == 'library' || $lineitemmeta['image_method'] == 'upload')){
...
echo "<fieldset id=\"fabricinfo\"";
if($mode=='workorderedit'){
    echo " class=\"readonlywrap\"";
    $fabricFieldsReadOnly=true;
}else{
    $fabricFieldsReadOnly=false;
}

echo "><legend>Fabric Information</legend>";
echo $this->Form->radio('fabric_type',['none'=>'No Fabric','existing'=>'Existing Fabric','typein'=>'Type-In Fabric'],['required'=>true,'value'=>$lineitemmeta['fabrictype'],'disabled'=>$fabricFieldsReadOnly,'readonly'=>$fabricFieldsReadOnly]);
...
if(($mode=='edit' || $mode=='workorderedit') && $lineitemmeta['fabrictype'] == 'existing'){
    echo " style=\"display:block;\">";
}else{
    echo " style=\"display:none;\">";
}
...
foreach($fabrics as $fabric){
	...
	if(($mode=='edit' || $mode=='workorderedit') && $lineitemmeta['fabrictype'] == 'existing' && $fabric['fabric_name'] == $lineitemmeta['fabricname']){ echo " selected=\"selected\""; }
...
if(($mode=='edit' || $mode=='workorderedit') && $lineitemmeta['fabrictype'] == 'typein'){
    echo " style=\"display:block;\">";
}else{
    echo " style=\"display:none;\">";
}
echo $this->Form->input('fabric_name',['label'=>'Fabric Name','autocomplete'=>'off','placeholder'=>'If Applicable','required'=>false,'value' => $lineitemmeta['fabric_name'],'readonly'=>$fabricFieldsReadOnly]);
echo $this->Form->input('fabric_color',['label'=>'Fabric Color','autocomplete'=>'off','placeholder'=>'If Applicable','required'=>false,'value'=>$lineitemmeta['fabric_color'],'readonly'=>$fabricFieldsReadOnly]);
...
$comcheckboxArr=['label'=>'COM Fabric','type'=>'checkbox','value'=>1,'readonly'=>$fabricFieldsReadOnly,'disabled'=>$fabricFieldsReadOnly];
if(($mode=='edit' || $mode=='workorderedit') && isset($lineitemmeta['com-fabric']) && $lineitemmeta['com-fabric']=='1'){
...
if($mode=='workorderedit' && isset($lineitemmeta['location'])){
    $thisLocation=$lineitemmeta['location'];
}else{
    $thisLocation=$thisLineItem['room_number'];
}

echo $this->Form->input('location',['label'=>'Location / Room Number','autocomplete'=>'off', 'placeholder' => 'If Applicable','value'=>$thisLocation]);
echo "</td>
...
if($mode=='edit' || $mode=='workorderedit'){
...
$(function(){
    ...
    $('#fabric-type-existing').click(function(){
		...
		$('#fabric-selector-block-mom').show('fast');
	...
	$('#fabric-type-typein').click(function(){
		...
		$('#fabric-selector-block-mom').hide('fast');
				/*PPSA -36 start */$('#fabric-selector-none').hide('fast');/*PPSA -36 end */
