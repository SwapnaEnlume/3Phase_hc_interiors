<!-- src/Template/Products/addbs.ctp -->
<h3>Add New Bedspread:</h3>
<?php
...
// 	echo "<option value=\"".$fabricname.";;".array_values($fabricdata)[0]['vendorId']."\">".$fabricname .$fabricVendorNameSpacing." [".array_values($fabricdata)[0]['vendorName']."]</option>\n";
	/* PPSASCRUM-100: end */
// }
// echo "</select>";
...
echo "
    <div class=\"custom-fabric-selection-dropdown\">
        ...
        ";
foreach($thefabrics as $fabricName => $fabricdata) {
	$vendorId = array_values($fabricdata)[0]['vendorId'];
	echo "
		<div
			id=\"" . array_values($fabricdata)[0]['id'] . "\" vendorId=\"" . $vendorId . "\"
			onclick=\"changeFabricColorOptions(event,'" . str_replace("'", "\'", $fabricName) . "', " . $vendorId . ", '" . str_replace("'", "\'", array_values($fabricdata)[0]['vendorName']) . "')\"
		>
			<span class=\"fabric-name\">" . $fabricName . "</span>
			<span class=\"vendor-name\">[" . array_values($fabricdata)[0]['vendorName'] . "]</span>
		</div>";
}
...
echo $this->Form->select('mattress_size',['36"'=>'36"','42"'=>'42"'],['value'=>'36"']);
echo "</div>";


echo $this->Form->input('description',['label'=>'Bedspread Description','type'=>'textarea','required'=>false]);
...
echo $this->Form->button('Add This Bedspread',['type'=>'submit']);
...
$(function(){
	$('fieldset input[type=checkbox]').change(function(){
		if($(this).prop('checked')){
			...
			$('#size-'+$(this).attr('data-sizeid')+'-price-wrap,#size-'+$(this).attr('data-sizeid')+'-weight-wrap,#size-'+$(this).attr('data-sizeid')+'-yards-wrap,#size-'+$(this).attr('data-sizeid')+'-difficulty-wrap').css({'opacity':'1.0','height':'auto'});
			$('#size-'+$(this).attr('data-sizeid')+'-price-wrap input,#size-'+$(this).attr('data-sizeid')+'-weight-wrap input,#size-'+$(this).attr('data-sizeid')+'-yards-wrap input,#size-'+$(this).attr('data-sizeid')+'-difficulty-wrap input').removeAttr('readonly');
			
		}else{
			
			$('#size-'+$(this).attr('data-sizeid')+'-price-wrap,#size-'+$(this).attr('data-sizeid')+'-weight-wrap,#size-'+$(this).attr('data-sizeid')+'-yards-wrap,#size-'+$(this).attr('data-sizeid')+'-difficulty-wrap').css({'opacity':'0.2'});
			$('#size-'+$(this).attr('data-sizeid')+'-price-wrap input,#size-'+$(this).attr('data-sizeid')+'-weight-wrap input,#size-'+$(this).attr('data-sizeid')+'-yards-wrap input,#size-'+$(this).attr('data-sizeid')+'-difficulty-wrap input').attr('readonly','readonly');
...
// 	$.get('/products/getfabriccolorscheckboxes/bs/'+encodeURIComponent(fabricname),
	$.get('/products/getfabriccolorscheckboxes/bs/'+encodeURIComponent(fabricName)+'/'+vendorId,
	/* PPSASCRUM-100: end */
	function(result){
