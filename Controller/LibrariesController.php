...
class LibrariesController extends AppController
{
			...
			case "add":
				if($this->request->data){
					...
					if($imgsTable->save($newImage)){
						...
						/*PPSASCRUM-185: start*/
	if (filesize($_SERVER['DOCUMENT_ROOT']."/webroot/img/library/".$filename) > 4 * 1024 * 1024) {
		$this->aspectratiofix($newImage->id, 600, 'image_library');
	} else {
		$this->aspectratiofix($newImage->id, false, 'image_library');
	}
	/*PPSASCRUM-185: end*/
			...
			case "edit":
				...
				if($this->request->data){
					//save changes
						...
						/**PPSASCRUM-263 start */
				if($this->request->data['new_image']['name'] != ''){
					...
					$allowedTypes=array('.jpg','jpeg','.png','.gif');
					if(!in_array(strtolower(substr($this->request->data['new_image']['name'],-4)),$allowedTypes)){
					    $this->autoRender=false;
					    echo '<h3><span style="color:red;">ERROR:</span> This filetype is not allowed.</h3>
                        <p><button type="button" onclick="history.go(-1);">Go Back</button></p>';
					    exit;
					...
					move_uploaded_file($this->request->data['new_image']['tmp_name'],$_SERVER['DOCUMENT_ROOT']."/webroot/img/library/".$filename);
				}
					/**PPSASCRUM-263 end */
				...
				if($this->request->data['new_image']['name'] != ''){	$imgChange->filename=$filename;
				}/**PPSASCRUM-263 start */
					...
					if($imgTable->save($imgChange)){
						...
						$this->resizeimage($_SERVER['DOCUMENT_ROOT']."/webroot/img/library/".$filename, $_SERVER['DOCUMENT_ROOT']."/webroot/img/library/" . $thumbnail[0] . '-small.' . $thumbnail[1], 200);
						if (filesize($_SERVER['DOCUMENT_ROOT']."/webroot/img/library/".$filename) > 4 * 1024 * 1024) {
							$this->resizefabricimageforeditpreview($_SERVER['DOCUMENT_ROOT']."/webroot/img/library/".$filename);
						}
					    	}
                        /**PPSASCRUM-263 end */
	...
	public function getimglist($status='active'){
		...
		foreach($imagefind as $image){
			...
			if (file_exists($_SERVER['DOCUMENT_ROOT'].'/webroot/img/library/' . $thumbnail[0] . '-small.' . $thumbnail[1])) {
				$thumb="<a href=\"/img/library/" . $thumbnail[0] . '-small.' . $thumbnail[1] . "\" class=\"fancybox\"><img src=\"/img/library/" . $thumbnail[0] . '-small.' . $thumbnail[1] . "\" width=\"85\" /></a>";
			} else {
				$thumb="<a href=\"/img/library/".$image['filename']."\" class=\"fancybox\"><img src=\"/img/library/".$image['filename']."\" width=\"85\" /></a>";
			}
			/*PPSASCRUM-185: end*/
