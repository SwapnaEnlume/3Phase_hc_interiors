...
use Cake\ORM\Table;
use Cake\Datasource\Exception\RecordNotFoundException; //PPSASCRUM-193
...
class ProductsController extends AppController
{
	...
	public function searchfabricsfield($type,$q){
	    ...
	    switch($type){
	        case 'cubicle-curtain':
	           ...
	           // $fabricFind=$this->Fabrics->find('all',['conditions' => ['OR' => ['use_in_cc' => '1', 'use_in_sc' => '1'], 'fabric_name LIKE' => '%'.$q.'%','status'=>'Active'],'order'=>['fabric_name'=>'asc']])->group('fabric_name')->toArray();
    			$fabricFind = $this->Fabrics->find('all',[
    				'conditions' => ['OR' => ['Fabrics.use_in_cc' => '1', 'Fabrics.use_in_sc' => '1'],'Fabrics.fabric_name LIKE' => '%'.$q.'%','Fabrics.status'=>'Active'],
    				'order'=>['Fabrics.fabric_name'=>'asc'],
    				'group' => ['Fabrics.fabric_name','Fabrics.vendors_id','vendors.vendor_name']])
    				->join([
    					'table' => 'vendors',
    					'type' => 'INNER',
    					'conditions' => 'Fabrics.vendors_id = vendors.id'
    				])
    				->select([
                        'id',
                        'fabric_name',
	        ...
	        case 'bedspread':
				...
				$fabricFind = $this->Fabrics->find('all',[
					'conditions' => ['Fabrics.use_in_bs' => '1','Fabrics.fabric_name LIKE' => '%'.$q.'%','Fabrics.status'=>'Active'],
					'order'=>['Fabrics.fabric_name'=>'asc'],
					'group' => ['Fabrics.fabric_name','Fabrics.vendors_id','vendors.vendor_name']])
					->join([
						'table' => 'vendors',
	        ...
	        case 'box-pleated':
				...
				$fabricFind = $this->Fabrics->find('all',[
					'conditions' => ['Fabrics.use_in_window' => '1','Fabrics.fabric_name LIKE' => '%'.$q.'%','Fabrics.status'=>'Active'],
					'order'=>['Fabrics.fabric_name'=>'asc'],
					'group' => ['Fabrics.fabric_name','Fabrics.vendors_id','vendors.vendor_name']])
					->join([
						'table' => 'vendors',
	    ...
	    foreach($fabricFind as $fabric){
	        ...
	        // $out[$fabric['id'] . " ;; " . $fabric['vendors_id']] = html_entity_decode($fabric['fabric_name'] . $fabricVendorNameSpacing . "[" . $fabric['vendor_name'] ."]");
	        $out[$fabric['id'] . " ;; " . $fabric['vendors_id']] = $fabric['fabric_name'] . " ;; [" . $fabric['vendor_name'] ."]";
			/* PPSASCRUM-100: end */
	    }
	...
	public function cubicleCurtains($cmdaction='',$itemid=0){
		...
		switch($cmdaction){
			...
			case "add":
				...
				$fabricsfind = $this->Fabrics->find('all',[
					'conditions' => ['OR' => ['Fabrics.use_in_cc' => 1, 'Fabrics.use_in_sc' => 1],'Fabrics.status'=>'Active'],
					'order'=>['Fabrics.fabric_name'=>'asc','Fabrics.color'=>'asc'],
					'group' => ['Fabrics.fabric_name', 'Fabrics.vendors_id', 'vendors.vendor_name']])
					->join([
						'table' => 'vendors',
						'type' => 'INNER',
				...
				// 	if(!in_array($fabric['fabric_name'],$usedfabrics)){
					if(!in_array($fabric['fabric_name'].",".$fabric['id'],$usedfabrics)){
				// 		$thefabrics[$fabric['fabric_name']]=array();
						$thefabrics[$fabric['fabric_name'].",".$fabric['id']]=array();
				// 		$usedfabrics[]=$fabric['fabric_name'];
						$usedfabrics[]=$fabric['fabric_name'].",".$fabric['id'];
					}
				// 	$thefabrics[$fabric['fabric_name']][$fabric['id']]=array('color'=>$fabric['color'],'id'=>$fabric['id'],'image'=>$fabric['image_file']);
					$thefabrics[$fabric['fabric_name'].",".$fabric['id']]=array('color'=>$fabric['color'],'id'=>$fabric['id'],'image'=>$fabric['image_file'],'vendorId'=>$fabric['vendors_id'],'vendorName'=>$fabric['vendor_name']);
					/* PPSASCRUM-100: end */
				}
...
// 		$conditions=array();
	...
	public function fabrics($subaction='default',$itemid=0,$fromtype='',$frominfo=''){
		$this->autoRender=false;
		/* PPSASCRUM-358: start */
		$pfasStatusMap = ['free-of-pfas' => 'Free of PFAS', 'contains-pfas' => 'Contains PFAS', 'not-verified' => 'Not Verified'];
		/* PPSASCRUM-358: end */
		switch($subaction){
			default:
			case "default":
			...
			case "add":
				...
				//	print_r($this->request->data);exit;
					...
					/* PPSASCRUM-358: start */
					$newFabric->pfas_status = $pfasStatusMap[$this->request->data['pfas_status']];
					/* PPSASCRUM-358: end */
							...
							if(isset($this->request->data['imageuploadfile']['tmp_name']) && strlen(trim($this->request->data['imageuploadfile']['tmp_name'])) >0){
								...
								switch($imagetype){
									...
									case "jpeg":
										...
										$newfile=$_SERVER['DOCUMENT_ROOT'].'/webroot/files/fabrics/'.$newFabric->id.'/'.str_replace('.jpeg','-small.jpeg',$newfilename);
									/*PPSASCRUM-185: end*/
									break;
									case ".gif":
								...
								/*PPSASCRUM-185: start*/
								if (filesize($_SERVER['DOCUMENT_ROOT']."/webroot/files/fabrics/".$newFabric->id."/".$newfilename) > 4 * 1024 * 1024) {
									$this->resizefabricimageforeditpreview($_SERVER['DOCUMENT_ROOT']."/webroot/files/fabrics/".$newFabric->id."/".$newfilename);
								}
								/*PPSASCRUM-185: end*/
			...
			case "edit":
				...
				if($this->request->data){
					...
					/* PPSASCRUM-358: start */
					$thisFabric->pfas_status = $pfasStatusMap[$this->request->data['pfas_status']];
					/* PPSASCRUM-358: end */
					...
					if(isset($this->request->data['new_image']['tmp_name']) && strlen(trim($this->request->data['new_image']['tmp_name'])) >0){
						...
						if(isset($this->request->data['new_image']['tmp_name']) && strlen(trim($this->request->data['new_image']['tmp_name'])) >0){
							...
							$thumbnail = explode('.', $fabricData['image_file']);
							@unlink($_SERVER['DOCUMENT_ROOT'].'/webroot/files/fabrics/'.$thisFabric['id'].'/'.$thumbnail[0] . '-small.' . $thumbnail[1]);
							/*PPSASCRUM-185: end*/
							...
							switch($imagetype){
								...
								case "jpeg":
									...
									$newfile=$_SERVER['DOCUMENT_ROOT'].'/webroot/files/fabrics/'.$thisFabric['id'].'/'.str_replace('.jpeg','-small.jpeg',$newfilename);
								/*PPSASCRUM-185: end*/
								break;
								case ".gif":
                            ...
                            if (filesize($_SERVER['DOCUMENT_ROOT']."/webroot/files/fabrics/".$thisFabric->id."/".$newfilename) > 4 * 1024 * 1024) {
								$this->resizefabricimageforeditpreview($_SERVER['DOCUMENT_ROOT']."/webroot/files/fabrics/".$thisFabric['id']."/".$newfilename);
							}
							$thisFabric->image_file = $newfilename;
                            /*PPSASCRUM-185: end*/  
						}
			...
			case "clone":
				...
				if($this->request->data){
					...
					if($liningTable->save($newLining)){
						...
						/* PPSASCRUM-299: start */
				// 		$this->Flash->success('Successfully added new lining "'.$this->request->data['title']);
						$this->Flash->success('Successfully cloned lining "'.$this->request->data['title'].'"');
				// 		$this->logActivity($_SERVER['REQUEST_URI'],"Added lining \"".$this->request->data['title']);
						$this->logActivity($_SERVER['REQUEST_URI'],"Cloned lining \"".$this->request->data['title']);
						/* PPSASCRUM-299: end */
						return $this->redirect('/products/linings/');
					}
				}else{					
	...
	public function getfabricslist(){
		$fabrics=array();
		$overallTotalRows=$this->Fabrics->find('all',['conditions' => ['Fabrics.status !=' => 'deleted']])->count();
		
		/* PPSASCRUM-358: start */
		$pfasStatuses = ['freeOfPfas' => 'Free of PFAS', 'containsPfas' => 'Contains PFAS', 'notVerified' => 'Not Verified'];
		/* PPSASCRUM-358: end */
					
		$conditions=array('Fabrics.status !=' => 'deleted');
		if(isset($this->request->data['search']['value']) && strlen(trim($this->request->data['search']['value'])) >0){
			//this is a search
			$conditions['OR']=array();
			$conditions['OR'] += array('fabric_name LIKE'=>'%'.trim($this->request->data['search']['value']).'%');
			...
			$conditions['OR'] += array('CONCAT(vendor_name,\' \',vendor_name) LIKE'=>'%'.trim($this->request->data['search']['value']).'%');
		...
		$fabricfind=$this->Fabrics->find('all',[
					...
					->select([
					    'id','pfas_status','color','image_file','cost_per_yard_bolt','cost_per_yard_cut','cut_price_last_updated',
					    'bolt_price_last_updated','case_price_last_updated','use_in_cc','use_in_bs','use_in_window',
					    'use_in_sc','antimicrobial','railroaded','vendors_id'
					    ,'flammability','vendor_fabric_name','vendor_color_name','description','status',
					    'fabric_width','fabric_name','cost_per_yard_case','collection',
       					'vendor_name' => 'vendors.vendor_name',
       					'vendor_status' => 'vendors.status'
					])->offset($this->request->data['start'])->limit($this->request->data['length'])->hydrate(false)->toArray();
		/* PPSASCRUM-358: end */
    
		$totalFilteredRows=$this->Fabrics->find('all',[
					'conditions' => $conditions,
					'order'=>$order])
					->join([
						'table' => 'vendors',
		...
		foreach($fabricfind as $fabric){
			...
			if(!file_exists($_SERVER['DOCUMENT_ROOT'].'/webroot/files/fabrics/'.$fabric['id'].'/'.$fabric['image_file'])){
				...
				$thumbnail = explode('.', $fabric['image_file']);
				$thumbnailFileName = $thumbnail[0] . '-small.' . $thumbnail[1];
				if(file_exists($_SERVER['DOCUMENT_ROOT'].'/webroot/files/fabrics/'.$fabric['id'].'/'.$thumbnailFileName)){
					$images='<img src="/files/fabrics/'.$fabric['id'].'/'.$thumbnailFileName.'" width="75" height="75" class="fabricimg" />';
				} else {
					$images='<img src="/files/fabrics/'.$fabric['id'].'/'.$fabric['image_file'].'" width="75" height="75" class="fabricimg" />';
				}
				/*PPSASCRUM-185: end*/
			...
			*/
			...
			if($fabric['vendors_id'] >0){
				...
				$vendor=$fabric['vendor_name'] ;//$thisVendor['vendor_name'];
			}else{
				$vendor='&nbsp;';
			}
			...
			/* PPSASCRUM-358: start */
			if ($fabricPFASStatus = $fabric['pfas_status']) {
				if ($fabricPFASStatus == $pfasStatuses['freeOfPfas']) {
					$pfasStatus = '<img src="/img/Ok-icon.png" width="16" height="16" />';
				} else if ($fabricPFASStatus == $pfasStatuses['containsPfas']) {
					$pfasStatus = '<img src="/img/delete.png" width="16" height="16" />';
				} else if ($fabricPFASStatus == $pfasStatuses['notVerified']) {
					$pfasStatus = '<img src="/img/not_verified.png" width="16" height="16" />';
				}
			} else {
				$pfasStatus = '<img src="/img/not_verified.png" width="16" height="16" />';
			}
			...
			$fabrics[]=array(
				...
				'1' => '<a href="/products/fabrics/edit/'.$fabric['id'].'/"><img src="/img/edit.png" alt="Edit This Fabric" title="Edit This Fabric" /></a> <a href="/products/fabrics/clone/'.$fabric['id'].'/"><img src="/img/clone.png" alt="Duplicate This Fabric" title="Duplicate This Fabric" /></a> <a href="/products/fabrics/delete/'.$fabric['id'].'/"><img src="/img/delete.png" alt="Delete This Fabric" title="Delete This Fabric" /></a>',
				'2' => $images,
				'3' => $fabric['id'],
				'4' => $fabric['fabric_name'].$vendorname.$description,
				'5' => $fabric['color'].$vendorcolor,
				'6' => '<b>'.$flammability.'</b>',
				/* PPSASCRUM-358: start */
				'7' => $pfasStatus,
				'8' => $fabric['fabric_width'],
				'9' => $railroaded,
				'10' => $antimicrobial,
				'11' => $vendor,
				'12' => $cost,
	...
	public function bedspreads($subaction='default',$bsid=0){
		...
		switch($subaction){
			...
			case "add":
				...
				$fabricsfind = $this->Fabrics->find('all',[
					'conditions' => ['Fabrics.use_in_bs' => '1','Fabrics.status'=>'Active'],
					'order'=>['Fabrics.fabric_name'=>'asc','Fabrics.color'=>'asc']])
					->join([
						'table' => 'vendors',
						'type' => 'INNER',
		...
		switch($cmdaction){
			case "add":
				if($this->request->data){
					...
					/*PPSASCRUM-124: start*/
mkdir(WWW_ROOT.'files/track-systems/'.$newTS->id);
/*PPSASCRUM-124: end*/
						if(isset($this->request->data['primary_image']['name']) && strlen(trim($this->request->data['primary_image']['name'])) >0){
						...
						/*PPSASCRUM-124: start*/
move_uploaded_file($this->request->data['primary_image']['tmp_name'],WWW_ROOT.'files/track-systems/'.$newTS->id.'/'.$newfilename);
/*PPSASCRUM-124: end*/
						}
			...
			case "delete":
			    /* PPSASCRUM-303: start */
				$thisTS = $this->TrackSystems->get($itemid);
				$thisTSArray = $thisTS->toArray();
				if ($this->request->data) {
					$thisTS->status = 'Inactive';
					if ($this->TrackSystems->save($thisTS)) {
						$this->Flash->success('Successfully deleted selected Track System');
						$this->logactivity($_SERVER['REQUEST_URI'], 'Deleted Track System "' . $thisTSArray['title'] . '"');
						$this->redirect('/products/track-systems/');
					}
				} else {
					$this->render('deletets');
				}
				/* PPSASCRUM-303: end */
			break;
			case "clone":
			    /* PPSASCRUM-300: start */
			    $trackSystemsTable = TableRegistry::get('TrackSystems');
				$cloneInputTrackSystem = $trackSystemsTable->get($itemid);

				if ($this->request->data) {
					$clonedTrackSystem = $trackSystemsTable->newEntity();
					$clonedTrackSystem->title = $this->request->data['title'];
					$clonedTrackSystem->status = $this->request->data['status'];
					$clonedTrackSystem->description = $this->request->data['description'];
					$clonedTrackSystem->qb_item_code = $this->request->data['qb_item_code'];
					$clonedTrackSystem->price = $this->request->data['price'];
					$clonedTrackSystem->system_or_component = $this->request->data['system_or_component'];
					$clonedTrackSystem->unit = $this->request->data['unit'];
					$clonedTrackSystem->inches_equivalent = $this->request->data['inches_equivalent'];
					if (isset($this->request->data['primary_image']['name']) && strlen(trim($this->request->data['primary_image']['name'])) > 0) {
						$filename = $this->request->data['primary_image']['name'];
						$filename = str_replace(",", "", $filename);
						$filename = str_replace("!", "", $filename);
						...
						$filename = str_replace(" ", "_", $filename);
						$newfilename = time() . "_" . $filename;
						$clonedTrackSystem->primary_image = $newfilename;
					}
					if ($trackSystemsTable->save($clonedTrackSystem)) {
						//handle image upload
						@mkdir(WWW_ROOT . 'files/track-systems/' . $clonedTrackSystem->id);
						if (isset($this->request->data['primary_image']['name']) && strlen(trim($this->request->data['primary_image']['name'])) > 0) {
							move_uploaded_file($this->request->data['primary_image']['tmp_name'], WWW_ROOT . 'files/track-systems/' . $clonedTrackSystem->id . '/' . $clonedTrackSystem->primary_image);
						}
						$this->Flash->success('Successfully cloned the Track System Component: ' . $cloneInputTrackSystem->title);
						$this->logActivity($_SERVER['REQUEST_URI'], "Cloned Track System Component \"" . $cloneInputTrackSystem->title . "\"");
						return $this->redirect('/products/track-systems/');
					}
				} else {
					$this->set('tsData', $cloneInputTrackSystem);
					$this->render('clonets');
				}
				/* PPSASCRUM-300: end */
			break;
	...
	public function bulkeditfabric($editfabrics){
		...
		if($this->request->data['process']=="step2"){
			...
			foreach($fixedparams as $fabricid){
				...
				/* PPSASCRUM-358: start */
				if (strlen($this->request->data['pfas_status']) > 0) {
					$pfasStatuses = ['free-of-pfas' => 'Free of PFAS', 'contains-pfas' => 'Contains PFAS', 'not-verified' => 'Not Verified'];
					$thisFabric->pfas_status = $pfasStatuses[$this->request->data['pfas_status']];
				}
				/* PPSASCRUM-358: end */
	...
	public function getspecialpricinglist($productType=false,$productID=false){
		...
		foreach($pricesfind as $price){
			...
			$specialPrices[]=array(
				...
				'4' => "<strong>".$title."</strong>", ////PPSASCRUM-193 
				'5' => $colors,
				'6' => $sizeData['title'],
			...
			case "step2":
				...
				switch($this->request->data['product_type']){
					case "cc":
						...
						$fabricsfind = $this->Fabrics->find('all',[
							'conditions' => ['OR' => ['Fabrics.use_in_cc' => '1', 'Fabrics.use_in_sc' => '1']],
							'order'=>['Fabrics.fabric_name'=>'asc'],
							'group'=>['Fabrics.fabric_name','Fabrics.vendors_id','vendors.vendor_name']])
							->join([
								'table' => 'vendors',
					...
					case "bs":
						...
						$fabricsfind = $this->Fabrics->find('all',[
							'conditions' => ['Fabrics.use_in_bs' => '1'],
							'order'=>['Fabrics.fabric_name'=>'asc'],
							'group'=>['Fabrics.fabric_name','Fabrics.vendors_id','vendors.vendor_name']])
							->join([
								'table' => 'vendors',
					...
					case "wt":
						...
						$fabricsfind = $this->Fabrics->find('all',[
							'conditions' => ['Fabrics.use_in_window' => '1'],
							'order'=>['Fabrics.fabric_name'=>'asc'],
							'group'=>['Fabrics.fabric_name','Fabrics.vendors_id','vendors.vendor_name']])
							->join([
								'table' => 'vendors',
		...
		switch($action){
			...
			case 'add':
				...
				if($this->request->data){
					...
					$allFabrics = $this->Fabrics->find('all',[
						'conditions' => ['Fabrics.is_hci_fabric' => 1, 'Fabrics.com_fabric' => 0],
						'order'=>['Fabrics.fabric_name'=>'asc','Fabrics.color'=>'asc']])
						->join([
							'table' => 'vendors',
							'type' => 'INNER',
...
// 	public function getfabriccolorscheckboxes($product=false,$fabricName){
	public function getfabriccolorscheckboxes($product=false,$fabricName,$vendorId=false){
	/* PPSASCRUM-100: end */
		$this->autoRender=false;
		$conditions=array();
		...
		if($product){
			switch($product){
				case "wt":
					/* PPSASCRUM-100: start */
					if ($vendorId) {
						$conditions += array('use_in_window'=>1,'vendors_id'=>$vendorId);						
					} else {
				...
				case "cc":
					/* PPSASCRUM-100: start */
					if ($vendorId) {
					    $conditions += array('OR'=>array('use_in_cc' => 1,'use_in_sc'=>1),'vendors_id'=>$vendorId);
					} else {
				...
				case "bs":
					/* PPSASCRUM-100: start */
					if ($vendorId) {
					    $conditions += array('use_in_bs' => 1,'vendors_id'=>$vendorId);
				    } else {
