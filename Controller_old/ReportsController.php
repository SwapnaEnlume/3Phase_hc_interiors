...
class ReportsController extends AppController
{
	...
	public function beforeFilter(Event $event)
    {
        parent::beforeFilter($event);
        	/**PPSA-40 start **/
		if($this->request->action=='msr' || $this->request->action=='bulkeditmsr' || $this->request->action == 'productiondetailbacklog' || $this->request->action=='backlogreport' || $this->request->action == 'bookingreport' || $this->request->action == 'backlogsummary' || $this->request->action == 'getmsrlist' || $this->request->action == 'consolidatedquotes' || $this->request->action=='bigquotes'|| $this->request->action=='quotesnewlines'){

			$this->Security->config('unlockedActions', ['msr','getmsrlist','backlogreport','bookingreport','backlogsummary','productiondetailbacklog','bulkeditmsr','consolidatedquotes','bigquotes','quotesnewlines']);
	...
	public function productiondetailbacklog(){
	    ...
	    if($this->request->data){
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('G1', 'FACILITY');
	...
	public function shippedordersdownload($start,$end){
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowNumber, 'FACILITY');
	...
	public function backlogreport($type='backlog'){
	    ...
	    if($this->request->data){
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('I1', 'FACILITY');
	...
	public function shipaddressreport(){
	    if($this->request->data){
			...
			//$orders=$this->Orders->find('all',['conditions'=>$conditions,'order'=>['order_number'=>'DESC']])->toArray();
			//SELECT *, ship_to.ship_to_name, facility.facility_name FROM orders LEFT JOIN ship_to ON orders.shipto_id=ship_to.id LEFT  JOIN facility ON orders.facility_id=facility.id;
			$db = ConnectionManager::get('default');
            $query = "SELECT * FROM orders ";
            $query.= "  JOIN ship_to ON orders.shipto_id=ship_to.id   ";
			$query.= "  JOIN facility ON orders.facility_id=facility.id ";
            $query.= " WHERE  ";
			...
			foreach($orders as $order){
				...
				if($order['shipto_id'] == NULL) {
					...
					$addressName = $order['ship_to_name'];
				}

				if($order['facility_id'] == NULL) {
					$facility = $order['facility'];
				} else {
					$addressName = $order['facility_name'];
				}
				...
				$customer=$this->Customers->find('all',['conditions'=>['id'=>$order['customer_id']]])->toArray();
			    ...
			    $objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, $order['attention']);
			    $objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, $order['shipping_address_1']);
			    $objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, $order['shipping_address_2']);
			    $objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, $order['shipping_city']);
			    $objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowCount, $order['shipping_state']);
			    $objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowCount, $order['shipping_zipcode']);
	...
	public function activeorders(){
		...
		if(isset($this->request->data['report_date_start']) && isset($this->request->data['report_date_end'])){
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('E1', 'FACILITY');
	...
	public function qbreconciliation(){
	    if($this->request->data){
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('H1', 'FACILITY');
...
public function quotesnewlines() {
        if ($this->request->data) {
            ...
            $query.= "IF(quotes.order_number IS NULL, quotes.title, orders.facility) AS FACILITY, ";
           ...
           // print_r($query);
            ...
            foreach ($ordersLookup as $orderRow) {
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue('G' . $rowCount, $orderRow['FACILITY']);
